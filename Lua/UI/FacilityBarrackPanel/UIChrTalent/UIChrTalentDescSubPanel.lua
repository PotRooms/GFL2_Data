...
function UIChrTalentDescSubPanel:RefreshByInbornTalentSkill()
  ...
  if talentKeyData.talent_key_type == 0 then
    IconUtils.GetSkillIconAsync(talentKeyData.BattleSkillId, self.ui.mImage_InbornTalentIcon)
  end
  ...
  if gunCmdData == nil or skillDisplayData == nil then
    return
  end
  self.ui.mTextFit_InbornSkillDesc.text = CS.GF2.Battle.SkillUtils.GetSkillDes(skillDisplayData.id, false, gunCmdData.gunGradeId, gunCmdData.TalentSkillIds)
end
...
function UIChrTalentDescSubPanel:RefreshByShareTalentItem()
  ...
  local talentGunData = TableDataBase.listSquadTalentGunDatas:GetDataById(self.gunId)
  IconUtils.GetItemIconSpriteAsync(talentGunData.FullyActiveItemId, self.ui.mImage_SkillTalentIcon)
  ...
  if isReceived then
    ...
    if isUnlocked then
      ...
      if talentGunData.unlock_sheet == 1 then
        local achievementDetailData = TableDataBase.listAchievementDetailDatas:GetDataById(talentGunData.unlock_id)
        if achievementDetailData == nil then
          return
        end
        self.ui.mText_Locked.text = achievementDetailData.des.str
      elseif talentGunData.unlock_sheet == 2 then
        local unlockDetailData = TableDataBase.listUnlockDetailDatas:GetDataById(talentGunData.unlock_id)
        if not unlockDetailData then
          return
        end
        self.ui.mText_Locked.text = unlockDetailData.des.str
      else
        gferror("SquadTalentGun\232\161\168unlock_sheet\229\173\151\230\174\181\228\184\173\229\173\152\229\156\168\230\156\170\229\174\154\228\185\137\231\154\132\232\167\163\233\148\129\231\177\187\229\158\139\239\188\154" .. talentGunData.unlock_sheet)
      end
    end
  end
end
...
function UIChrTalentDescSubPanel:showSkill(talentKeyId)
  ...
  if talentKeyData.BattleSkillId > 0 then
    local skillDisplayData = TableData.listBattleSkillDisplayDatas:GetDataById(talentKeyData.BattleSkillId)
    local gunCmdData = NetCmdTeamData:GetGunByID(self.gunId)
    local skillDes = CS.GF2.Battle.SkillUtils.GetSkillDes(skillDisplayData.id, false, gunCmdData.gunGradeId, gunCmdData.TalentSkillIds)
    self.ui.mTextFit_TalentSkillDesc.text = skillDes
...
function UIChrTalentDescSubPanel:showSkillDiff(curLvSkillId, curLevel, nextLvSkillId)
  ...
  local gunCmdData = NetCmdTeamData:GetGunByID(self.gunId)
  local skillDes = CS.GF2.Battle.SkillUtils.GetSkillDes(curLvSkillDisplayData.id, false, gunCmdData.gunGradeId, gunCmdData.TalentSkillIds)
  self.ui.mText_CurSkillDesc.text = skillDes
  self.ui.mText_NextLevel.text = TableData.GetHintReplaceById(80057, tostring(curLevel + 1))
  local nextSkillDes = CS.GF2.Battle.SkillUtils.GetSkillDes(nextLvSkillDisplayData.id, false, gunCmdData.gunGradeId, gunCmdData.TalentSkillIds)
  self.ui.mText_NextSkillDesc.text = nextSkillDes
...
function UIChrTalentDescSubPanel:refreshGrpBtn(talentState, talentType)
  ...
  elseif talentState == UITalentGlobal.TalentState.Unauthorized then
    setactive(self.ui.mTrans_AuthorizeRedPoint, NetCmdTalentData:IsNeedRedPointOfTalentPoint(self.gunId, self.groupId))
...
function UIChrTalentDescSubPanel:showProperty(attributeItemTable, propertyId, template)
  ...
  local usedIndex = 1
  for j = startPropertyId + 1, endPropertyId - 1 do
...
function UIChrTalentDescSubPanel:onClickAuthorization()
  ...
  for itemId, cost in pairs(groupData.cost) do
    ...
    if cost > count then
      ...
      TipsPanelHelper.OpenUITipsPanel(itemData, nil, true)
      return
    end
  end
  ...
  elseif talentState == UITalentGlobal.TalentState.Unauthorized then
    ...
    for itemId, cost in pairs(kvPairList) do
      if itemId == 2 then
        ...
        IconUtils.GetItemIconSpriteAsync(itemId, self.ui.mImage_CostIcon2)
  ...
  elseif talentState == UITalentGlobal.TalentState.Unauthorized then
    ...
    for itemId, cost in pairs(kvPairList) do
      if itemId == 2 then
        ...
        IconUtils.GetItemIconSpriteAsync(itemId, self.ui.mImage_CostIcon1)
...
function UIChrTalentDescSubPanel:refreshSlotIcon(geneData, talentType)
  if talentType == UITalentGlobal.TalentType.NormalAttribute then
    self:setPropertyIconAsync(geneData.PropertyId, self.ui.mImage_PropertyTalentIcon)
  ...
  elseif talentType == UITalentGlobal.TalentType.PrivateTalentKey then
    ...
    IconUtils.GetItemIconSpriteAsync(geneData.ItemId, self.ui.mImage_SkillTalentIcon)
  end
end
...
function UIChrTalentDescSubPanel:setPropertyIconAsync(propertyId, image)
  local propertyTypeName = PropertyHelper.GetOnlyOnePropty(propertyId)
  local propertyData = TableData.GetPropertyDataByName(propertyTypeName)
  IconUtils.GetAttributeIconAsync(propertyData.icon, image)
end
