...
function UIChrTalentSlot:ctor(root, onClickCallback)
  ...
  self.ui.mText_RandomNum.text = "00\239\188\154" .. UIUtils.AddZeroFrontNum(5, randomNum)
end
...
function UIChrTalentSlot:Refresh()
  ...
  if self.level == 1 and self.level > self.preLevel then
    if self.type == UITalentGlobal.TalentType.NormalAttribute or self.type == UITalentGlobal.TalentType.AdvancedAttribute then
      self.ui.mAnimator:SetTrigger("ComItem_Fx")
    elseif self.type == UITalentGlobal.TalentType.PrivateTalentKey then
      self.ui.mAnimator:SetTrigger("SkillItem_Fx")
    end
  ...
  if self.type == UITalentGlobal.TalentType.NormalAttribute then
    ...
    self:setPropertyIconAsync(geneData.PropertyId, self.ui.mImage_PropertyIcon)
  ...
  elseif self.type == UITalentGlobal.TalentType.PrivateTalentKey then
    ...
    IconUtils.GetItemIconSpriteAsync(geneData.ItemId, self.ui.mImage_SkillItemIcon)
  end
  if NetCmdTalentData:IsNeedRedPointOfTalentPoint(self.gunId, self.groupId) then
...
function UIChrTalentSlot:setPropertyIconAsync(propertyId, image)
  local propertyTypeName = PropertyHelper.GetOnlyOnePropty(propertyId)
  local propertyData = TableData.GetPropertyDataByName(propertyTypeName)
...
function UIChrTalentSlot:onClickSlot()
  if self.gunId == nil then
    gfdebug("\229\189\147\229\137\141\230\152\175\231\169\186\229\164\169\232\181\139\231\130\185!")
    return
  end
