...
function UIGunTalentAssemblyPanel:OnShowStart()
  self:RefreshSlotCount()
end

function UIGunTalentAssemblyPanel:RefreshSlotCount()
  ...
  if privateTalentSlotCount == 0 then
    for i, slot in ipairs(self.privateSlotList) do
  ...
  elseif privateTalentSlotCount == 1 then
    ...
    for i, slot in ipairs(self.privateSlotList) do
      if i == 2 then
        local data = privateSlotDataList[dataIndex]
        slot:SetVisible(true)
        slot:SetData(true, data, self.mGunId, i - 1)
      else
        slot:SetVisible(false)
      end
    end
  ...
  if publicTalentSlotCount == 0 then
    for i, slot in ipairs(self.publicSlotList) do
      slot:SetVisible(false)
    end
  elseif publicTalentSlotCount == 1 then
    ...
    for i, slot in ipairs(self.publicSlotList) do
      if i == 2 then
        local data = publicSlotDataList[dataIndex]
        slot:SetVisible(true)
        slot:SetData(false, data, self.mGunId, i - 1)
      else
        slot:SetVisible(false)
      end
    end
...
function UIGunTalentAssemblyPanel:OnClickSlotItem(slotItem, needScroll)
  if needScroll == nil then
    needScroll = true
  end
  gfdebug("OnClickSlotItem: " .. slotItem.slotId)
  ...
  self:InitPluginList(slotItem, nil, needScroll)
end
  ...
  elseif assemblyType == self.AssemblyType.Replace then
    if self.curSlotItem.isPrivate then
      if data.ownerId == 0 or data.ownerId == self.mGunId then
        ...
        gfwarning("\228\184\147\229\177\158\230\138\128\232\131\189\230\143\146\228\187\182\228\184\141\229\143\175\232\131\189\229\177\158\228\186\142\229\133\182\228\187\150\228\186\186\229\189\162\239\188\129")
      end
    elseif data.ownerId == 0 or data.ownerId == self.mGunId then
      NetCmdTalentData:SendChangePublicSkillsItemBelong(self.mGunId, self.curSlotItem.slotId, data.uId, function()
        UIUtils.PopupPositiveHintMessage(180014)
...
function UIGunTalentAssemblyPanel:InitPluginList(slotItem, templist, needScroll)
  if needScroll == nil then
    needScroll = true
  end
  ...
  if not self.curSlotItem.isPrivate and needScroll then
    self:SetScroll()
  end
end
...
function UIGunTalentAssemblyPanel:RefreshPluginInfo(pluginData)
  ...
  if pluginBattleSkillData then
    local skillDes = CS.GF2.Battle.SkillUtils.GetSkillDes(pluginBattleSkillData.id, false, self.mGunCmdData.gunGradeId, self.mGunCmdData.TalentSkillIds)
    self.ui.mText_SkillDescription.text = skillDes
...
function UIGunTalentAssemblyPanel:PrivatePluginsSlotCallBack(msg)
  ...
  self:OnClickSlotItem(self.curSlotItem, false)
end
