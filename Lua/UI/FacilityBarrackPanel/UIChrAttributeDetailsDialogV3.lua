...
function UIChrAttributeDetailsDialogV3:InitProp()
  ...
  self.ui.mScrollRect_AttributeList.verticalNormalizedPosition = 1
end
...
function UIChrAttributeDetailsDialogV3:UpdateWeaknessDescribe(elementlist)
  ...
  for i = 0, tmpParentTrans.childCount - 1 do
    setactive(tmpParentTrans:GetChild(i).gameObject, false)
  end
  local weaponTagDatas = CS.WeakUIUtils.SetGunWeakWeaponTag(self.gunData.id, tmpParentTrans, false)
  local weaponTagDatasCount = 0
  if weaponTagDatas ~= nil then
    weaponTagDatasCount = weaponTagDatas.Count
  end
  local weakTagsCount = 0
  if elementlist == nil or #elementlist == 0 then
    weakTagsCount = 0
  else
    if 0 < #elementlist and tmpParentTrans.childCount < #elementlist + weaponTagDatasCount then
      instantiate(tmpItem, tmpParentTrans, false)
    end
    local weakTags = CS.WeakUIUtils.SetGunWeakTag(self.gunData.id, tmpParentTrans, false, weaponTagDatasCount)
    if weakTags ~= nil then
      weakTagsCount = weakTags.Length
    end
  end
  local allCount = weaponTagDatasCount + weakTagsCount
  if allCount == 0 then
    self:ShowWeaknessDescribe(false)
    return
  else
