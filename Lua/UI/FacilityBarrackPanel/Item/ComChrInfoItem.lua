...
function ComChrInfoItem:InitCtrl(parent, obj)
...
function ComChrInfoItem:InitCtrlNoAddList(ScrollListChild, parent)
  local instObj = instantiate(ScrollListChild.childItem, parent)
  self.ui = {}
  self:LuaUIBindTable(instObj, self.ui)
  self:SetRoot(instObj.transform)
  self.dutyItem = UICommonDutyItem.New()
...
function ComChrInfoItem:InitCtrlWithoutInstantiate(instObj)
  self.ui = {}
  self:LuaUIBindTable(instObj, self.ui)
  self:SetRoot(instObj.transform)
  self.dutyItem = UICommonDutyItem.New()
...
function ComChrInfoItem:SetData(gunCmdData, gunData, callback)
  ...
  self.ui.mBtn_Self.enabled = callback ~= nil
  ...
  if self.isUnLock then
    ...
    if isFight then
      ...
      self.ui.mText_LevelNum.text = self.mGunCmdData.fightingCapacity
    else
