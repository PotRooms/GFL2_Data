...
function UISkillDetailItem:SetGunCmdData(gunCmdData)
  self.gunCmdData = gunCmdData
end

function UISkillDetailItem:SetData(skillData, curLevel, textColor, showBottomBtn, curGunGradeId)
  ...
  if skillData then
    ...
    if self.gunCmdData ~= nil then
      local skillDetialInfo = self.gunCmdData:GetPreviewGunSkillDetialInfoBySkillId(skillData.id)
      local tmpDesc = self.gunCmdData:FormatSkillDesc(skillData.id, skillData.upgrade_description.str, skillDetialInfo)
      self.mText_Desc.text = tmpDesc
    else
