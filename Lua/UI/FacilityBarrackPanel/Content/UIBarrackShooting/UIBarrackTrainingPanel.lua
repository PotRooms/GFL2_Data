...
function UIBarrackTrainingPanel:OnInit(root, gunId)
  ...
  function self.onItemUpdateCallback()
    self:OnRefresh()
  end
  
  MessageSys:AddListener(CS.GF2.Message.CommonEvent.ItemUpdate, self.onItemUpdateCallback)
end
...
function UIBarrackTrainingPanel:OnClose()
  ...
  MessageSys:RemoveListener(CS.GF2.Message.CommonEvent.ItemUpdate, self.onItemUpdateCallback)
end
...
function UIBarrackTrainingPanel:OnCameraStart()
  local showTypeId = LuaUtils.EnumToInt(self.mCSPanel.ShowType)
  if showTypeId == 0 then
...
function UIBarrackTrainingPanel:setData(gunId)
  self.gunId = gunId
