...
function UIChrSkillInfoDialog:OnInit(root, data)
  ...
  if data.curGunGradeId == nil then
    self.curGunGradeId = self.gunCmdData.upgrade
  else
    self.curGunGradeId = data.curGunGradeId
  end
  self.curTabItem = nil
  self.curTabIndex = 0
...
function UIChrSkillInfoDialog:UpdateSkillInfo()
  ...
  local skillDetialInfo = self.gunCmdData:GetPreviewGunUpGradeSkillDetialInfo(self.curGunGradeId)
  local tmpDesc = self.gunCmdData:FormatSkillDesc(self.skillData.id, nil, skillDetialInfo) .. "\n"
  ...
  if 0 < elementTag then
    ...
    self.ui.mText_Element.text = elementData.name.str
  end
  self.elementTag = elementTag
  self:UpdateWeak()
  setactive(self.ui.mImg_Element, 0 < elementTag)
  setactive(self.ui.mText_Element, 0 < elementTag)
  setactive(self.ui.mTrans_Element1, 0 < elementTag)
...
function UIChrSkillInfoDialog:UpdateSkillListInfo()
  ...
  if isMaxLevel and self.skillData.Level == 1 then
    ...
    local skillDetialInfo = self.gunCmdData:GetPreviewGunUpGradeSkillDetialInfo(self.curGunGradeId)
    local tmpDesc = self.gunCmdData:FormatSkillDesc(self.skillData.id, self.skillDisplayData.DescriptionBrief.str, skillDetialInfo)
    self.ui.mTextFit_Describe.text = tmpDesc
  end
  local tmpList = {}
  for i = 0, skillList.Count - 1 do
    ...
    if tmpList[skillList[i]] == nil and skillData.level ~= 1 then
      item:SetGunCmdData(self.gunCmdData)
      item:SetData(skillData, curSkillLevel, nil, self.showBottomBtn, self.curGunGradeId)
...
function UIChrSkillInfoDialog:OnClickTabBtn(index)
  ...
  self.ui.mScrollRect_SkillDescription.verticalNormalizedPosition = 1
...
function UIChrSkillInfoDialog:UpdateWeak()
  self.ui.mTrans_Element1.transform:SetSiblingIndex(0)
  local parentTrans = self.ui.mTrans_Element
  local weaponStcId = self.gunCmdData.WeaponStcId
  if parentTrans.childCount < 2 then
    local obj = parentTrans:GetChild(0).gameObject
    instantiate(obj, parentTrans)
  end
  local weak = CS.WeakUIUtils.SetSkillWeakWeapon(weaponStcId, self.skillData.id, parentTrans, false, 1)
  self.ui.mTrans_Element1.transform:SetSiblingIndex(parentTrans.childCount - 1)
  setactive(parentTrans, weak ~= nil and 0 < weak.Count or 0 < self.elementTag)
end
