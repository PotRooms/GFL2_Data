...
function UIRaidDialogV2:OnInit(root, data)
  ...
  self.ui.mText_Tip.text = TableData.GetHintById(193009)
  if data.hintID then
    self.formatString = TableData.GetHintById(data.hintID)
  end
  if data and data.textStr then
    self.ui.mText_Tip.text = data.textStr
  end
  self.currentCostItemCount = 0
  self.itemList = {}
  self.originalColor = self.ui.mText_CostNum.color
  local isShowCurrency = true
  if data.isShowCurrency ~= nil then
    isShowCurrency = data.isShowCurrency
  end
  setactive(self.ui.mTrans_CurrencyRoot, isShowCurrency)
  self:AddBtnListener()
...
function UIRaidDialogV2:RefreshRewardList()
  ...
  for i, v in ipairs(self.mData.rewardItemList) do
    local itemID = v.id
    local itemNum = self.itemDataTable[itemID]
    if self.itemList[i] == nil then
      self.itemList[i] = UICommonItem.New()
      self.itemList[i]:InitCtrl(self.ui.mTrans_RewardContent)
    end
    local item = self.itemList[i]
    local escortData = NetCmdItemData:GetEScortExchangeData(itemID)
    if escortData and NetCmdActivitySimData.IsOpenFakeLogic then
      local fakeItemData = CS.UICommonItem.GetFakeItemData(escortData, itemNum)
...
function UIRaidDialogV2:onClickStartRaid()
  ...
  if GlobalConfig.StaminaId == self.mData.costItemId and not TipsManager.CheckStaminaIsEnoughOnly(self.mData.costItemNum * self.curRaidTimes) then
    return
  end
