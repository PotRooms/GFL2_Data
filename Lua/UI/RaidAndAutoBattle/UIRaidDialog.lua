...
function UIRaidDialog:refreshCostText()
  ...
  if self:isStaminaEnough(self.curRaidTimes) then
    self.ui.mText_CostNum.color = CS.GF2.UI.UITool.StringToColor("325563")
  else
...
function UIRaidDialog:onClickStartRaid()
  ...
  local stageTypeId = LuaUtils.EnumToInt(StageType.CashStage)
  if self.simEntranceData.id == CS.LuaUtils.EnumToInt(StageType.CashStage) then
    local itemDataTable = {}
...
function UIRaidDialog:getRemainingChallengeTimes()
  if not self.simEntranceData then
    gferror("\230\149\176\230\141\174\228\184\186\231\169\186!")
    return 0
  end
