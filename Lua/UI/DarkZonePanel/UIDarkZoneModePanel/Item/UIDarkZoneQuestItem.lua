...
function UIDarkZoneQuestItem:SetData(data, questBundleList, state, curLevelType, frontFinish, enoughLevel)
  ...
  self.ui.mText_Type.text = data.quest_tag
  local level = self:GetDepartIndex()
  if level then
    if level == 0 then
    ...
    elseif level == 2 then
      self.ui.mImg_icon.color = DarkZoneGlobal.ColorType.newBie3
      self.ui.mImg_TypeBg.color = DarkZoneGlobal.ColorType.newBie3
      self.ui.mImg_ChestProcess.color = DarkZoneGlobal.ColorType.newBie3
    end
  else
  ...
  if self.tiemrState ~= nil then
    self.tiemrState:Stop()
    self.tiemrState = nil
  end
  self.tiemrState = TimerSys:DelayFrameCall(3, function()
    self.ui.mAni_QuestItem:SetInteger("State", state)
  end)
end

function UIDarkZoneQuestItem:GetDepartIndex()
  local i = -1
  for key, value in pairs(NetCmdDarkZoneSeasonData.currentDepartToGroup) do
    local id = key
