...
function UIDarkZoneModePanel:OnInit(root, data)
  ...
  self.modeScheduleID = data.modeScheduleID
...
function UIDarkZoneModePanel:CloseSelfUI()
  UIManager.CloseUI(UIDef.UIDarkZoneModePanel)
end

function UIDarkZoneModePanel:InitDarkZoneGlobalLevel()
  ...
  local questBundleList = {}
  local modeToDepart = NetCmdDarkZoneSeasonData.modeToDepartDict[self.modeScheduleID]
  for modeIndex = 0, modeToDepart.Count - 1 do
    local Depart = modeToDepart[modeIndex]
    local groupList = NetCmdDarkZoneSeasonData.SeasonDepartToGroupList[Depart]
    for i = 0, groupList.Count - 1 do
      table.insert(questBundleList, groupList[i])
    end
    local departToGroup = NetCmdDarkZoneSeasonData.SeasonDepartToGroupList[Depart]
    DarkZoneGlobal.DepartToLevel[Depart] = {}
    table.insert(DarkZoneGlobal.DepartList, Depart)
    for i = 0, departToGroup.Count - 1 do
      table.insert(DarkZoneGlobal.DepartToLevel[Depart], departToGroup[i])
    end
  end
  for index, key in pairs(questBundleList) do
    if DarkZoneGlobal.LevelTypeMin == 0 then
      DarkZoneGlobal.LevelTypeMin = index
    end
    DarkZoneGlobal.LevelTypeMax = index
    DarkZoneGlobal.LevelToStcID[index] = key
    DarkZoneGlobal.StcIDToLevel[key] = index
  end
end
...
function UIDarkZoneModePanel:SetTabType(tabType)
  ...
  if self.hasFinishGroupList and self.selectTab == DarkZoneGlobal.PanelType.Explore and not self:IsExploreUnlock() then
    ...
    CS.PopupMessageManager.PopupString(TableData.listDarkzoneSeasonDatas:GetDataById(NetCmdDarkZoneSeasonData.SeasonID).explore_unlock_des.str)
  elseif self.hasFinishGroupList and self.selectTab == DarkZoneGlobal.PanelType.EndLess and not self:IsEndlessUnlock() then
    self.selectTab = DarkZoneGlobal.PanelType.Quest
    CS.PopupMessageManager.PopupString(TableData.listDarkzoneSeasonDatas:GetDataById(NetCmdDarkZoneSeasonData.SeasonID).endless_unlock_des.str)
  end
end
