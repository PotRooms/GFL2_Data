...
function UIDarkZoneQuestPanelV2:InitCtrl(prefab, parent, parentPanel)
  ...
  self.modeScheduleID = parentPanel.modeScheduleID
  self.finishSeriesQuest = DarkNetCmdStoreData.seriesQuest.FinishQuest
  NetCmdDarkZoneSeasonData.currentDepartToGroup = NetCmdDarkZoneSeasonData:GetModeDepartToGroup(self.modeScheduleID)
  NetCmdDarkZoneSeasonData.currentGroupToQuest = NetCmdDarkZoneSeasonData:GetModeGroupToQuest(NetCmdDarkZoneSeasonData.currentDepartToGroup)
  self.departToGroup = NetCmdDarkZoneSeasonData.currentDepartToGroup
  self.groupToQuest = NetCmdDarkZoneSeasonData.currentGroupToQuest
  ...
  if DarkNetCmdStoreData.questCacheGroupId ~= 0 and DarkZoneGlobal.StcIDToLevel[DarkNetCmdStoreData.questCacheGroupId] ~= nil then
    gfdebug("DarkNetCmdStoreData.questCacheGroupId :" .. tostring(DarkNetCmdStoreData.questCacheGroupId))
  ...
  gfdebug("self.curLevelType " .. tostring(self.curLevelType))
end
...
function UIDarkZoneQuestPanelV2:UpdateLock()
  ...
  if pcall(function()
    ...
    gferror("\232\175\187\232\161\168\229\135\186\231\142\176\233\151\174\233\162\152\239\188\129\239\188\129\232\175\183\230\163\128\230\159\165\232\161\168\230\160\188")
  end
end
...
function UIDarkZoneQuestPanelV2:UpdateBtnLock()
  for k, v in pairs(DarkZoneGlobal.DepartToLevel) do
    ...
    self.btnList[k].btnUI.mAnimator_Root:SetBool("Locked", not self.btnList[k].isUnlock)
  end
end
...
function UIDarkZoneQuestPanelV2:UpdateBtnFinish()
  for k, v in pairs(DarkZoneGlobal.DepartToLevel) do
    ...
    setactive(self.btnList[k].btnUI.mTrans_Compelete, self.btnList[k].isFinish)
  end
end
...
function UIDarkZoneQuestPanelV2:Show(isFirstShow, isBackFrom, isNeedDelayFadein, isOnRecover)
  ...
  local time = 0.2
  if isOnRecover then
    isNeedDelayFadein = true
    time = DarkZoneGlobal.PopDelayTimeZone
  end
  self:ShowCurTypeList(unMoveAnimation, true, isNeedDelayFadein, isBackFrom, isOnRecover, time)
end
...
function UIDarkZoneQuestPanelV2:SetQuestItemData(prefabList, GroupId)
  ...
  for i = 1, 4 do
    ...
    if systemQuestData and i <= #systemQuestData then
    ...
    if systemQuestData and i <= #systemQuestData and self.hasFinishQuestList[systemQuestData[i].id] then
...
function UIDarkZoneQuestPanelV2:OnLeftClick()
  ...
  if not self:CheckLevelUnlock(tempLevel) then
    ...
    if level == 99999999 and questID == 0 then
      self:OnRightClick()
      return
    end
    if level > self.userLevel then
...
function UIDarkZoneQuestPanelV2:ShowCurTypeList(unMoveAnimation, isNeedFadeIn, isNeedDelayFadein, isBackFrom, isOnRecover, DalayTime)
  ...
  local isunlock, flag = self:CheckLevelUnlock(posIndex + 3)
  if flag == -1 then
    setactive(self.ui.mTrans_Lock3, false)
  end
  isunlock, flag = self:CheckLevelUnlock(posIndex + 2)
  if flag == -1 then
    setactive(self.ui.mTrans_Lock2, false)
  end
  ...
  elseif unMoveAnimation then
    ...
    if isNeedFadeIn then
      if isNeedDelayFadein then
        self.timerMapFadein = TimerSys:DelayCall(DalayTime, function()
          self:SetAnimatorFadeIn()
          self.timerMapFadein = nil
        end)
      else
...
function UIDarkZoneQuestPanelV2:UpdateMapLevelBg(posIndex)
  ...
  if self.curLevelType - 1 == posIndex then
    ...
    if DarkZoneGlobal.LevelToStcID[self.curLevelType + 1] == nil then
  ...
  if self:CheckBtnAllUnlock(self.questType) and 2 <= #DarkZoneGlobal.DepartToLevel[self.questType] then
    setactive(self.ui.mBtn_PreGun.transform.parent.transform, true)
    setactive(self.ui.mBtn_NextGun.transform.parent.transform, true)
  end
  local colorIndex = self:GetDepartIndex()
  ...
  elseif colorIndex == 2 then
    self.ui.mImg_MapBg.color = DarkZoneGlobal.ColorType.newBie3
  end
end
...
function UIDarkZoneQuestPanelV2:GetDepartIndex()
  local i = -1
  for key, value in pairs(self.departToGroup) do
    local id = key
    local left = DarkZoneGlobal.StcIDToLevel[DarkZoneGlobal.DepartToLevel[id][1]]
    local right = left
    i = i + 1
    for j = 2, #DarkZoneGlobal.DepartToLevel[id] do
      right = DarkZoneGlobal.StcIDToLevel[DarkZoneGlobal.DepartToLevel[id][j]]
    end
    if left <= self.curLevelType and right >= self.curLevelType then
      return i
    end
  end
  return false
...
function UIDarkZoneQuestPanelV2:UpdateBtnRedPoint(LevelStcID)
  ...
  for i = 1, #seasonQuestList do
    ...
    if self:CheckLevelUnlock(DarkZoneGlobal.StcIDToLevel[questBundle.QuestGroup]) and isUnlock == false then
      count = count + 1
    end
  end
  return count
...
function UIDarkZoneQuestPanelV2:Release()
  ...
  MessageSys:RemoveListener(UIEvent.DZRefreshChest, self.RefreshItem)
