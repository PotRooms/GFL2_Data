...
function UIPVPRankRewardItem:SetData(data, index)
  ...
  setactive(self.ui.mTrans_Max.gameObject, data.LevelId == NetCmdPVPData.historyMaxLevel)
end
...
function UIPVPRankRewardItem:SetRewards(rewardList)
  ...
  for i, v in ipairs(tmpRewardList) do
    ...
    item:SetReceivedIcon(NetCmdPVPData.historyMaxLevel >= self.nrtpvpLevelData.LevelId)
  end
  self:SetRewardState()
end
