    ...
    if self.pvpHistoryInfo and self.pvpHistoryInfo.NrtPvpHistory.Opponent and self.pvpHistoryInfo.NrtPvpHistory.Opponent.DefendGuns then
      ...
      if defendData and defendData.Avatars then
        ...
        if self.pvpOpponentInfo.uid == 0 then
          local gunPresetDatas = TableDataBase.listGunPresetDatas:GetDataById(defendData.Avatars[0].Id)
          if gunPresetDatas then
            id = gunPresetDatas.SourceId
          end
        end
...
function UIPVPChallengeItemV2:SetItemState()
  ...
  local desc = string.format("id = %d \231\177\187\229\158\139 = %d \231\187\147\230\158\156 = %d", self.pvpOpponentInfo.uid, self.detailType, self.pvpOpponentInfo.result)
  print(desc)
  if self.detailType == UIPVPGlobal.ButtonType.Challenge then
