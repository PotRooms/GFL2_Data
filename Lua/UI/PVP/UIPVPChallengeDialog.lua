...
function UIPVPChallengeDialog:OnInit(root)
  ...
  function self.RefreshChallenge(msg)
    self:OnRefreshMatchCallBack(msg)
  end
  
  MessageSys:AddListener(UIEvent.PVPChallengeRefresh, self.RefreshChallenge)
  MessageSys:AddListener(CS.GF2.Message.UIEvent.OnLoadingEnd, self.UpdateBonusAnimation)
  
  function UIPVPChallengeDialog.ConnectSuccess()
    UIPVPChallengeDialog.isRefreshSucc = true
  end
  
  MessageSys:AddListener(CS.GF2.Message.UIEvent.OnReconnectSuc, UIPVPChallengeDialog.ConnectSuccess)
end
...
function UIPVPChallengeDialog:CurWeeklySettleIsClose()
  self.isClose = true
...
function UIPVPChallengeDialog:CleanCloseTimer()
...
function UIPVPChallengeDialog:PVPCheckKickOut()
...
function UIPVPChallengeDialog:OnRefreshChallenge()
...
function UIPVPChallengeDialog:OnShowFinish()
  ...
  if not NetCmdPVPData.opponentDailyRefresh then
    NetCmdPVPData:ReqNrtPvpOpponentDailyRefresh(function(ret)
    end)
  else
    self:OnRefreshMatchCallBack(true)
  end
  self:OnRefreshChallenge()
  self:PVPCheckKickOut()
end
...
function UIPVPChallengeDialog:RefreshListAnim()
  if NetCmdPVPData:GetChallengeAni() then
    NetCmdPVPData:SetChallengeAni(false)
...
function UIPVPChallengeDialog:OnClickRefreshMatchList()
  ...
  if self.isRefreshSucc == false then
    return
  end
  self.isRefreshSucc = false
  ...
  if freeTimes <= 0 then
    ...
    if resNum < self.itemCost then
      ...
      local content = MessageContent.New(hint, MessageContent.MessageType.DoubleBtn, function()
        NetCmdPVPData:ReqRefreshMatch(true, function(ret)
          self.isRefreshSucc = true
        end)
      end, function()
        self.isRefreshSucc = true
    ...
    NetCmdPVPData:ReqRefreshMatch(false, function(ret)
      self.isRefreshSucc = true
    end)
  end
end
...
function UIPVPChallengeDialog:OnRefreshMatchCallBack(isHideMessage)
  self.pvpMatchInfo = NetCmdPVPData.PvpMatchInfo
  print("self.isAllComplete =============" .. tostring(self.isAllComplete))
  if not isHideMessage then
    if self.isAllComplete then
      CS.PopupMessageManager.PopupStateChangeString(TableData.GetHintById(120059))
    else
...
function UIPVPChallengeDialog:UpdateCountDownTime()
  ...
  self:ReleaseTimer()
  self.pvpChallengeTimer = TimerSys:DelayCall(1, function()
    timerFunc()
  end, nil, repeatCount)
end
...
function UIPVPChallengeDialog:ReleaseTimer()
  if self.pvpChallengeTimer then
    self.pvpChallengeTimer:Stop()
    self.pvpChallengeTimer = nil
  end
end
...
function UIPVPChallengeDialog:OnClose()
  ...
  self:CleanCloseTimer()
  MessageSys:RemoveListener(UIEvent.PVPChallengeRefresh, self.RefreshChallenge)
  MessageSys:RemoveListener(CS.GF2.Message.UIEvent.OnLoadingEnd, self.UpdateBonusAnimation)
  MessageSys:RemoveListener(CS.GF2.Message.UIEvent.OnReconnectSuc, UIPVPChallengeDialog.ConnectSuccess)
end
