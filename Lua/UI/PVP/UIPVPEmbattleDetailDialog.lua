...
function UIPVPEmbattleDetailDialog:SetData(data, detailType)
  ...
  self.ui.mText_Title.text = self.pvpMapData.MapName.str
  self.ui.mText_Num2.text = UIPVPGlobal.PvpCostNum
  if GlobalData.GetStaminaResourceItemCount(UIPVPGlobal.NrtPvpTicket) < 1 then
    self.ui.mText_Num2.color = ColorUtils.RedColor
  else
    self.ui.mText_Num2.color = ColorUtils.WhiteColor
  end
...
function UIPVPEmbattleDetailDialog:RefreshDropUp()
  if self.mTrans_Up == nil then
    self.mTrans_Up = self.ui.mBtn_GrpStart.transform:Find("Root/Trans_ItemModeIcon")
  end
  local enable = NetCmdActivityDropUpData:GetActivityUpCountMaxByStageType(LuaUtils.EnumToInt(StageType.NrtpvpStage))
  setactivewithcheck(self.mTrans_Up, enable)
end
...
function UIPVPEmbattleDetailDialog:SetOpponentInfo()
  ...
  if self.pvpOpponentInfo.uid > 0 then
    ...
    self.playerAvatar:SetData(TableData.GetPlayerAvatarIconById(self.pvpOpponentInfo.user.Portrait, LuaUtils.EnumToInt(self.pvpOpponentInfo.user.Sex)))
    ...
    self.ui.mText_PlayerName.text = pvpDummyData.name.str
  ...
  self.playerAvatar:AddBtnListener(function()
    LuaUtils.OpenUIPlayerInfoDialog(AccountNetCmdHandler:GetRoleInfoData())
  end)
...
function UIPVPEmbattleDetailDialog:CurWeeklySettleIsClose()
...
function UIPVPEmbattleDetailDialog:CleanCloseTimer()
...
function UIPVPEmbattleDetailDialog:CheckKickOut()
  self:CleanCloseTimer()
...
function UIPVPEmbattleDetailDialog:OnShowFinish()
  self:CheckKickOut()
end
