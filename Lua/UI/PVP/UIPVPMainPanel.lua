...
function UIPVPMainPanel:RefreshDropUp()
  local enable = NetCmdActivityDropUpData:GetActivityUpCountMaxByStageType(LuaUtils.EnumToInt(StageType.NrtpvpStage))
  setactivewithcheck(self.ui.mTrans_Up, enable)
  local up = NetCmdActivityDropUpData:PvpDropUpTimes()
  if math.floor(up) == up then
    up = math.floor(up)
  end
  self.ui.mText_Up.text = enable and string_format(TableData.GetHintById(260214), up) or ""
end
...
function UIPVPMainPanel:SetRewardItemWithIndex(i)
  ...
  UIUtils.GetButtonListener(self.ui["mBtn_RewardItem" .. i].gameObject).onClick = function()
    if UIPVPGlobal.CurRewardState[i] == UIPVPGlobal.RewardType.Receive then
      ...
      local canGetList = {}
      for j = 1, 3 do
        if UIPVPGlobal.CurRewardState[j] == UIPVPGlobal.RewardType.Receive then
          table.insert(canGetList, j)
        end
      end
      if 1 < #canGetList then
        NetCmdPVPData:ReqAllReward(canGetList, function()
...
function UIPVPMainPanel:UpdateLeftTabList()
  ...
  local systemID = LuaUtils.EnumToInt(SystemList.NrtpvpNewSupplyTime)
