...
function UICommonSettingItem:InitCtrl(parent, callback, setToFirst)
  ...
  self.ui.mBtn_Self = obj:GetComponent(typeof(CS.UnityEngine.UI.GFButton))
  self:__InitCtrl()
  self.btnList = {}
...
function UICommonSettingItem:SetData(data, parent)
  ...
  elseif data.type == 3 then
    ...
    UIUtils.GetButtonListener(self.ui.mBtn_Use.gameObject).onClick = function()
      local voice = CS.OssVoiceSetLog(data.id + 1)
      MessageSys:SendMessage(OssEvent.VoiceSetLog, nil, voice)
...
function UICommonSettingItem:RefreshScreenPos()
  if self.mData.id == UISettingSubPanel.GraphicSettings.Resolution then
    ...
    self.ui.mList_Screen.transform.offsetMin = Vector2(0, -545)
  end
end
...
function UICommonSettingItem:RefreshPos()
  if self.mData.id ~= UISettingSubPanel.GraphicSettings.Resolution then
    ...
    local isDown = not self.isUp
    ComScreenItemHelper:RefreshFilterTransPos(self.ui.mTrans_BtnScreen, tmpScreen, isDown)
  end
end
...
function UICommonSettingItem:InitScreen(data)
  ...
  if data.id == UISettingSubPanel.GraphicSettings.Resolution then
    self.ui.mList_Screen = instantiate(UIUtils.GetGizmosPrefab("UICommonFramework/ComSettingViewModeDropDownListItemV2.prefab", self), self.ui.mTrans_BtnScreen)
  else
    self.ui.mList_Screen = instantiate(UIUtils.GetGizmosPrefab("UICommonFramework/ComSettingDropDownListItemV2.prefab", self), self.ui.mTrans_BtnScreen)
  end
  self:LuaUIBindTable(self.ui.mList_Screen, self.ui)
  self.blockHelper = UIUtils.GetUIBlockHelper(self.parent.mUIRoot, self.ui.mList_Screen.transform, function()
...
function UICommonSettingItem:RefreshVoice()
  ...
  setactive(self.ui.mTrans_Download.gameObject, false)
  setactive(self.ui.mTrans_Use.gameObject, AccountNetCmdHandler.AvgVoice ~= self.mData.id)
end
...
function UICommonSettingItem:SetInteractable(bool)
  self.ui.mBtn_Screen.interactable = bool
end
