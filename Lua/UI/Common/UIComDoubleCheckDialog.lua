...
function UIComDoubleCheckDialog:SetData()
  ...
  setactive(self.ui.mImgTop.gameObject, false)
  setactive(self.ui.mBtn_BtnCancel.gameObject, self.param.isDouble ~= nil and self.param.isDouble)
  if self.param.topResources ~= nil and self.topRes == nil then
    self.topRes = UITopResourceBar.New()
    self.topRes:Init(self.mUIRoot, self.param.topResources, true)
...
function UIComDoubleCheckDialog:UpdateResourceBar()
  local currencyParent = CS.TransformUtils.DeepFindChild(self.mUIRoot, "Root/GrpTop/GrpCurrency")
  ...
  if currencyParent ~= nil then
    self.topRes:UpdateCurrencyContent(currencyParent, self.param.topResources)
  end
end
