...
function UIHardChapterSelectPanelV2:OnInit(root, data)
  ...
  self.canRaidNum = self.tableTotalRaidNum
...
function UIHardChapterSelectPanelV2:RefreshRaidNum()
  self.canRaidNum = self.tableTotalRaidNum - NetCmdSimulateBattleData.storyTotalRaidNum
  self.ui.mText_TotalRaidNum.text = string_format(TableData.GetHintById(112016), self.canRaidNum, self.tableTotalRaidNum)
end
...
function UIHardChapterSelectPanelV2:EnterHard(item)
  if item.mData.id and item.isUnLock then
    ...
    UIManager.OpenUIByParam(UIDef.UIHardChapterDetailPanel, {
      chapterId = chapterId,
      stageType = LuaUtils.EnumToInt(StageType.DifficultStage)
    })
  end
end
