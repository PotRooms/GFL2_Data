...
function UIBranchChapterListItem:SetData(data)
  ...
  if data ~= nil then
    ...
    self.nextStory = nil
    self.nextBranchStory = nil
    if self.ui.mAnimator.isDestroyed == false then
      self.ui.mAnimator:SetBool("Locked", false)
    end
    if not self.isUnlock and self.ui.mAnimator.isDestroyed == false then
...
function UIBranchChapterListItem:UpdateBranch()
  ...
  if stageData ~= nil then
    ...
    self.ui.mText_BranchName.text = self.storyData.name
    ...
    setactive(self.ui.mTrans_Star, self.storyData.type ~= GlobalConfig.StoryType.StoryBattle and self.storyData.type ~= GlobalConfig.StoryType.Teach and 0 < stageData.challenge_list.Count)
    setactive(self.ui.mTrans_Tutorial, self.storyData.type == GlobalConfig.StoryType.Teach)
    if self.isUnlock then
      if stageRecord then
        if self.storyData.type == GlobalConfig.StoryType.Teach then
