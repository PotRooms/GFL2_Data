...
function UIHardChapterDetailPanel:OnClose()
  self.ui.mText_StaminaCost.color = self.originalColor
...
function UIHardChapterDetailPanel:InitBaseData()
  ...
  self.originalColor = self.ui.mText_StaminaCost.color
  self.dropList = {}
  self.firstDropList = {}
  self.enemyList = {}
  self.challengeList = {}
...
function UIHardChapterDetailPanel:UpdateRaidInfo()
  ...
  setactive(self.ui.mTrans_RiddleItemList, false)
end
...
function UIHardChapterDetailPanel:UpdatePanel()
  ...
  local firstRewardShow = 0 < self.stageData.more_drop_view_list.Count
  ...
  if self.StaminaCostPercent == nil then
    ...
    if 0 < costItem then
      ...
      if 0 < costItem then
        ...
        self.ui.mText_StaminaCost.text = CS.LuaUIUtils.GetMaxNumberText(self.stamincost)
        self.ui.mText_StaminaCost.color = self.costItemNum < self.stamincost and ColorUtils.RedColor or self.originalColor
      end
    end
  else
  ...
  for i, dropItem in ipairs(sortedDropList) do
    ...
    if itemData ~= nil and analysisItemID[itemId] == nil then
      if self.dropList[index] == nil then
        local item1 = self:GetAppropriateItem(false)
      ...
      if itemData.type ~= GlobalConfig.ItemType.Weapon then
        item:SetExtraIconVisible(isExtra or dropItem.isFirst and not self.isFirst)
      end
      index = index + 1
    else
    end
  end
  sortedDropList = self:GetItemSortNew(firstDropList)
  for i, v in ipairs(self.firstDropList) do
    v:SetActive(false)
  end
  index = 1
  for i, dropItem in ipairs(sortedDropList) do
...
function UIHardChapterDetailPanel:UpdateRaidDropItemList()
  local index = 1
  local showData = UIUtils.GetKVSortItemTable(self.storyData.sweeps_reward)
  for i, v in ipairs(self.riddleItemList) do
    v:SetActive(false)
  end
...
function UIHardChapterDetailPanel:OnClickEnemy(enemyId)
  local enemyData = TableData.GetEnemyData(enemyId)
  CS.RoleInfoCtrlHelper.Instance:InitSysEnemyDataByStage(enemyId, self.stageData.stage_class + enemyData.add_level, self.stageData.id)
end
...
function UIHardChapterDetailPanel:ClearUpItems()
...
function UIHardChapterDetailPanel:SetActUpVisible()
  self.stageType = self.stageType or 32
  self.upItemList = {}
