...
function UICommandCenterHudPanel:OnInit(root, data)
  ...
  RedPointSystem:GetInstance():AddRedPointListener(RedPointConst.TeachPPT, self.mItem_TeachPPT.transRedPoint, nil, self.mItem_TeachPPT.systemId)
...
function UICommandCenterHudPanel:InitCommandCenterPanelUI()
  ...
  self.topTabItemList = {}
  self.mItem_TeachPPT = self:InitCommandCenterTopBtn(SystemList.Ppt, "PPT")
  ...
  local isCurBpOpen = NetCmdBattlePassData:CheckCurBpIsOpen()
  setactive(self.mItem_BattlePass.btn.gameObject, isCurBpOpen and AccountNetCmdHandler:CheckSystemIsUnLock(SystemList.Battlepass))
...
function UICommandCenterHudPanel:InitButtonGroup()
  ...
  UIUtils.GetButtonListener(self.mItem_TeachPPT.btn.gameObject).onClick = function()
    self:OnClickTeachPPT()
  end
...
function UICommandCenterHudPanel:InitKeyCode()
  ...
  self:RegistrationKeyboard(KeyCode.I, self.mItem_PlayerInfo.btn)
...
function UICommandCenterHudPanel:OnShowFinish()
  SceneSys.CurrentSingleScene:SetSceneGaussianBlur(1)
  self:CheckBackground()
  self:UpdatePlayerInfo()
  self:UpdateRedPoint()
  setactive(self.ui.mTrans_ActivitieOpen, NetCmdThemeData:GetThemeRedState() ~= 3 and NetCmdThemeData:GetActivityIsOpen())
  self:UpdateSystemUnLockInfo()
  self:InitChatContent()
  self:InitKeyCode()
  SceneSys.CurrentSingleScene:ShowProjection(true)
end
...
function UICommandCenterHudPanel:OnTop()
  ...
  if root then
    self:RefreshActivityEntrance()
    self.isFadeIn = true
    self.fadeInTimer = TimerSys:DelayCall(1, function()
      self.isFadeIn = false
    end)
...
function UICommandCenterHudPanel:OnClose()
  ...
  RedPointSystem:GetInstance():RemoveRedPointListener(RedPointConst.TeachPPT)
...
function UICommandCenterHudPanel:OnClickStore()
...
function UICommandCenterHudPanel:OnClickPost()
  if TipsManager.NeedLockTips(SystemList.Notice) or self.isFadeIn then
    return
  end
  self:UnRegistrationAllKeyboard()
  SceneSys.CurrentSingleScene:SetSceneGaussianBlur(0)
...
function UICommandCenterHudPanel:OnClickTeachPPT()
  self:UnRegistrationAllKeyboard()
  SceneSys.CurrentSingleScene:SetSceneGaussianBlur(0)
  self:CallWithAniDelay(function()
    local param = CS.ShowTeachPPTData()
    param.GroupId = 0
    param.IsInBattle = false
    UIManager.OpenUIByParam(CS.GF2.UI.enumUIPanel.UIGuidePPTDialog, param)
  end)
end

function UICommandCenterHudPanel:OnClickCheckIn()
  if TipsManager.NeedLockTips(SystemList.Checkin) or self.isFadeIn then
    return
  end
  self:UnRegistrationAllKeyboard()
  SceneSys.CurrentSingleScene:SetSceneGaussianBlur(0)
...
function UICommandCenterHudPanel:OnClickMail()
  if TipsManager.NeedLockTips(SystemList.Mail) or self.isFadeIn then
    return
  end
  self:UnRegistrationAllKeyboard()
  SceneSys.CurrentSingleScene:SetSceneGaussianBlur(0)
...
function UICommandCenterHudPanel:OnClickActivity()
  if TipsManager.NeedLockTips(SystemList.Activity) or self.isFadeIn then
    return
  end
  self:UnRegistrationAllKeyboard()
  if NetCmdOperationActivityData:HasShowingActivity() then
    ...
    SceneSys.CurrentSingleScene:ShowProjection(false)
    self:CallWithAniDelay(function()
      UIManager.OpenUIByParam(UIDef.UIActivityDialog, {})
    end)
  else
...
function UICommandCenterHudPanel:OnClickBattleSim()
  ...
  self:CallWithAniDelay(function()
    ...
    UISystem:JumpByID(menuData.jump)
  end)
end
...
function UICommandCenterHudPanel:OnClickChat()
  self:UnRegistrationAllKeyboard()
  SceneSys.CurrentSingleScene:SetSceneGaussianBlur(0)
  SceneSys.CurrentSingleScene:MovePuppy(true)
