...
function UICommandCenterPanelV3:InitCommandCenterPanelUI()
  ...
  self.mItem_Exchange = self:InitCommandCenterBottomBtn(self.ui.mBtn_BtnStoreExchange, SystemList.Store)
...
function UICommandCenterPanelV3:OnRecover(data, behaviorId, isTop)
  ...
  if SceneSys.CurrentSingleScene.ResetMainCamera ~= nil then
    SceneSys.CurrentSingleScene:ResetMainCamera()
  end
  if isTop then
...
function UICommandCenterPanelV3:OnHideFinish()
  if SceneSys.CurrentSingleScene.StopConversation ~= nil then
    SceneSys.CurrentSingleScene:StopConversation()
  end
  self:OnHideSlideShow()
end
...
function UICommandCenterPanelV3:NickPlayerName()
  if not AccountNetCmdHandler:GetRecordFlag(GlobalConfig.RecordFlag.NameModified) then
    ...
    data.ChapterId = storyData.chapter
...
function UICommandCenterPanelV3:SendCheckInCallback(ret)
  if NetCmdCheckInData:IsChecked() then
    ...
    UIManager.OpenUIByParam(UIDef.UIDailyCheckInPanel, function()
      if SceneSys.CurrentAdditiveSceneType ~= EnumSceneType.CommandCenter then
...
function UICommandCenterPanelV3:SystemUnLock()
  printstack("\230\156\137\230\150\176\231\154\132\231\179\187\231\187\159\232\167\163\233\148\129\228\186\134\229\165\165")
  self:UpdateSystemUnLockInfo()
end
