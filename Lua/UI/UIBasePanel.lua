...
function UIBasePanel:LuaUIBindTable(target, UITable, index)
  if UITable == nil then
    gfdebug("table\232\161\168\228\184\186\231\169\186")
    return
  end
  ...
  for i = 0, vars.Count - 1 do
    local varItem = vars[i]
    if index ~= nil then
      UITable[varItem .. index] = LuaUIBindScript:GetBindingComponent(varItem)
    else
      UITable[varItem] = LuaUIBindScript:GetBindingComponent(varItem)
    end
  end
end
...
function UIBasePanel:SetVisible(visible)
  if not self.mUIRoot or not self.mUIRoot.gameObject then
    return
  end
...
function UIBasePanel:ReleaseCtrlTable(ctrlTable, isDestroy)
  ...
  for k, uiBaseCtrl in pairs(ctrlTable) do
    uiBaseCtrl:OnRelease(isDestroy)
    ctrlTable[k] = nil
  end
end
