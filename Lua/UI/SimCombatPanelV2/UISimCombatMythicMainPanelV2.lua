...
function UISimCombatMythicMainPanelV2:OnInit(root, param)
  ...
  for i = 0, TableData.listSimCombatMythicGroupDatas.Count - 1 do
    ...
    if data.group_type ~= 1 then
      table.insert(self.chapterItemDataList, data)
    end
  end
  if 0 < NetCmdSimCombatMythicData.CurUnlimitedConfigId then
    local data = TableData.listSimCombatMythicGroupDatas:GetDataById(NetCmdSimCombatMythicData.CurUnlimitedConfigId)
    table.insert(self.chapterItemDataList, data)
  end
  ...
  UIUtils.GetButtonListener(self.ui.mBtn_BtnRight.gameObject).onClick = function()
    if self.curChapterItem.itemIndex ~= self.chapterItemDataList[#self.chapterItemDataList].id then
      self:OnClickChapterItem(self.stageGroupItemList[self.curChapterItem.itemIndex + 1])
    end
  end
  ...
  function self.OnNewIdMainPanel()
    if NetCmdSimCombatMythicData.CurUnlimitedConfigId > 0 then
      local data = TableData.listSimCombatMythicGroupDatas:GetDataById(NetCmdSimCombatMythicData.CurUnlimitedConfigId)
      local needAdd = true
      for i = 1, #self.chapterItemDataList do
        if self.chapterItemDataList[i].group_type == 1 then
          self.chapterItemDataList[i] = data
          needAdd = false
        end
      end
      if needAdd then
      ...
      local delData = false
      local index = -1
      for i = #self.chapterItemDataList, 1, -1 do
        if self.chapterItemDataList[i].group_type == 1 then
          table.remove(self.chapterItemDataList, i)
          delData = true
          index = i
        end
      end
      if delData then
        table.remove(self.stageGroupItemList, index)
        table.remove(self.chapterTextItemList, index)
      end
    end
    self:InitChapterList()
  end
  
  MessageSys:AddListener(UIEvent.OnMythicNewUnlimitedConfigId, self.OnNewIdMainPanel)
end
...
function UISimCombatMythicMainPanelV2:OnShowFinish()
  ...
  self:InitChapterList()
  NetCmdSimCombatMythicData:GetUnlimitedConfigId()
end
...
function UISimCombatMythicMainPanelV2:InitChapterList()
  for i = self.ui.mTrans_Content.childCount - 1, 0, -1 do
    setactive(self.ui.mTrans_Content:GetChild(i), false)
  end
  for i = self.ui.mTrans_NumContent.childCount - 1, 0, -1 do
    setactive(self.ui.mTrans_NumContent:GetChild(i), false)
  end
...
function UISimCombatMythicMainPanelV2:SetDefaultSelectGroup()
  TimerSys:DelayFrameCall(2, function()
    ...
    local cfgData = TableData.listSimCombatMythicGroupDatas:GetDataById(autoSlcGroupIndex)
    local defaultItem = self.stageGroupItemList[autoSlcGroupIndex]
    if cfgData.group_type == 1 then
      defaultItem = self.stageGroupItemList[#self.stageGroupItemList]
    end
...
function UISimCombatMythicMainPanelV2:OnClickChapterItem(item)
  ...
  self:ScrollToPosByIndex(item.itemIndex, function()
    ...
    setactive(self.ui.mBtn_BtnRight.gameObject, self.curChapterItem.itemIndex ~= self.chapterItemDataList[#self.chapterItemDataList].id)
  end)
end
...
function UISimCombatMythicMainPanelV2:RemoveListener()
  MessageSys:RemoveListener(UIEvent.OnMythicNewUnlimitedConfigId, self.OnNewIdMainPanel)
end
