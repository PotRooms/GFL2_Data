...
function UISimCombatMythicStageGroupDetailPanel:InitData(isOver)
  ...
  if isOver == nil then
    ...
    function self.OnNewIdDetailPanel()
      if self.groupConfigData.group_type == 1 then
        self:ShowResetPopMsg()
      end
    end
  end
end
...
function UISimCombatMythicStageGroupDetailPanel:InitLeftTab()
  for i = 0, self.ui.mScrollListChild_GrpLeftList.transform.childCount - 1 do
    UIUtils.Destory(self.ui.mScrollListChild_GrpLeftList.transform:GetChild(i))
  end
...
function UISimCombatMythicStageGroupDetailPanel:UpdateRightPanel()
  ...
  self.ui.mText_StageLevelContent.text = stageLevelConfig.level_desc.str
...
function UISimCombatMythicStageGroupDetailPanel:SetTaskEnemyInfo(stageTaskId)
  ...
  if enemiesCount < #self.enemyIconItems then
    for i = enemiesCount + 1, #self.enemyIconItems do
      self.enemyIconItems[i]:SetData(nil)
    end
  end
  for i = 1, enemiesCount do
    ...
    UIUtils.GetButtonListener(item.mBtn_OpenDetail.gameObject).onClick = function()
      CS.RoleInfoCtrlHelper.Instance:InitSysEnemyDataByStage(enemyId, stageData.stage_class + enemyData.add_level, stageData.id)
    end
  end
end
...
function UISimCombatMythicStageGroupDetailPanel:SetTaskRewardInfo(stageTaskId)
  local state = NetCmdSimCombatMythicData:GetStageLevelState(self.groupID, self.curSelectedStageLevelIndex)
  local isReceived = state == SimCombatMythicConfig.StageLevelState.FINISH_ADVANCE or state == SimCombatMythicConfig.StageLevelState.FINISH_BASE
  ...
  for k, v in pairs(reward) do
    ...
    item:SetReceivedIcon(isReceived)
    item:SetFirstDrop(not isReceived)
    index = index + 1
  end
end
...
function UISimCombatMythicStageGroupDetailPanel:GetAutoSelectedLevelItem()
  ...
  local stageLevelIndex = autoSlcItem:GetStageLevelIndex()
  local state = NetCmdSimCombatMythicData:GetStageLevelState(self.groupID, stageLevelIndex)
  if SimCombatMythicConfig.refightSelectedStageId ~= nil and state ~= SimCombatMythicConfig.StageLevelState.UNLOCK then
    autoSlcItem = self.stageLevelsItemList[SimCombatMythicConfig.refightSelectedStageId]
  end
  return autoSlcItem
end
...
function UISimCombatMythicStageGroupDetailPanel:OnClickStageLevelItem(item)
  ...
  SimCombatMythicConfig.refightSelectedStageId = item:GetStageLevelIndex()
...
function UISimCombatMythicStageGroupDetailPanel:AddListener()
  ...
  MessageSys:AddListener(UIEvent.OnMythicNewUnlimitedConfigId, self.OnNewIdDetailPanel)
end
...
function UISimCombatMythicStageGroupDetailPanel:RemoveListener()
  ...
  MessageSys:RemoveListener(UIEvent.OnMythicNewUnlimitedConfigId, self.OnNewIdDetailPanel)
end
...
function UISimCombatMythicStageGroupDetailPanel:ShowResetPopMsg()
  MessageBox.Show(TableData.GetHintById(64), TableData.GetHintById(103192), nil, function()
    self:ResetPanelInfo()
  end, function()
    self:ResetPanelInfo()
  end)
...
function UISimCombatMythicStageGroupDetailPanel:ResetPanelInfo()
  if NetCmdSimCombatMythicData.CurUnlimitedConfigId < 0 then
    UIManager.CloseUI(UIDef.UISimCombatMythicStageGroupDetailPanel)
    return
  end
  local cfgData = TableData.listSimCombatMythicGroupDatas:GetDataById(NetCmdSimCombatMythicData.CurUnlimitedConfigId)
  self.groupID = cfgData.Id
