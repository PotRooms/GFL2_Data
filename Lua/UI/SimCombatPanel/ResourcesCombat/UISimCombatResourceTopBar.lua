...
function UISimCombatResourceTopBar:SetData(simEntranceId, simCombatTypeId)
  ...
  self:refreshTimes()
  setactivewithcheck(self.ui.mBtn_Hint, true)
end
...
function UISimCombatResourceTopBar:refreshTimes()
  setactivewithcheck(self.ui.mTrans_GrpExtra, false)
  setactivewithcheck(self.ui.mTrans_GrpChallenge, false)
  setactivewithcheck(self.ui.mTrans_TextDec, false)
  ...
  if timerData == nil or simTypeData.extra_drop_cost == 0 then
    setactivewithcheck(self.ui.mText_GrpExtraReward, false)
  ...
  if simTypeData.extra_drop_cost ~= 0 and time then
    ...
    if haveMonCard then
      ...
      setactivewithcheck(self.ui.mTrans_TextDec, true)
    else
      local text = "<color=#FFFFFF>" .. haveNum .. "</color>/" .. time
      self.ui.mText_ExtraNum.text = text
    end
    if haveNum ~= 0 then
      setactivewithcheck(self.ui.mTrans_GrpExtra, true)
    end
  else
    local itemID = self.simCombatEntranceData.item_id
    if itemID ~= 0 then
      setactivewithcheck(self.ui.mTrans_GrpChallenge, true)
...
function UISimCombatResourceTopBar:onClickHint()
  MessageBox.Show(TableData.GetHintById(64), TableData.GetHintById(103189), MessageBox.ShowFlag.eMidBtn, nil, nil, nil)
end
