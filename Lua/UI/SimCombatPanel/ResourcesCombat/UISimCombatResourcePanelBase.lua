...
function UISimCombatResourcePanelBase:OnInit(root, data, behaviourId)
  ...
  function self.onCloseCommonReceivePanel()
    self.stageList:SetLayoutDoneDirty()
  end
  
  MessageSys:AddListener(UIEvent.OnCommonReceivePanelClickConfirm, self.onCloseCommonReceivePanel)
end
...
function UISimCombatResourcePanelBase:OnClose()
  ...
  MessageSys:RemoveListener(UIEvent.OnCommonReceivePanelClickConfirm, self.onCloseCommonReceivePanel)
  self.topBar:OnClose()
...
function UISimCombatResourcePanelBase:IsReadyToStartTutorial()
  gfinfo("IsReadyToStartTutorial: " .. NetCmdItemData:GetUserDropCache():ToString() .. "    levelDelta: " .. AccountNetCmdHandler.mLevelDelta)
  local dropCache = NetCmdItemData:GetUserDropCache()
  if dropCache.Count > 0 then
    return false
  end
...
function UISimCombatResourcePanelBase:refresh(fromState)
  ...
  local enumId = LuaUtils.EnumToInt(PlanType.PlanFunctionSimDailyopen)
  NetCmdSimulateBattleData:ReqPlanData(enumId, function()
...
function UISimCombatResourcePanelBase:checkShowRaidOpenHint()
  ...
  if self.simEntranceId ~= enumId then
    return
  end
...
function UISimCombatResourcePanelBase:onClickTab(simTypeId, isOpenDay, fromState)
  self.stageList:SetData(simTypeId, self.jumpSlotResourceId, isOpenDay, fromState)
  self.stageList:Refresh()
end
