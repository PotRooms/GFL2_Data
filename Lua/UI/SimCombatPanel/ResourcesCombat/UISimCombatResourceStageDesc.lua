...
require("UI.SimCombatPanel.Item.SimCombatResourceUpItem")
...
function UISimCombatResourceStageDesc:ctor(root)
  ...
  UIUtils.AddBtnClickListener(self.ui.mBtn_Raid.gameObject, function()
    if self.hasUp and NetCmdActivityDropUpData:NewDay() then
      local content = MessageContent.New(TableData.GetHintById(260179), MessageContent.MessageType.DoubleBtn, function()
        self:onClickRaid()
      end, function()
        MessageBoxPanel.Close()
      end, TableData.GetHintById(260178))
  ...
  self:RegisterEvent()
  self.defaultCostColor = self.ui.mText_CostNum.color
end
...
function UISimCombatResourceStageDesc:SetData(simEntranceId, simResourceData, isOpenDay, curSlotIndex)
  ...
  self.simCombatCfg = TableData.listSimCombatResourceDatas:GetDataById(self.simResourceData.id)
  self.simCombatTypeCfg = TableData.listSimCombatTypeDatas:GetDataById(self.simCombatCfg.sim_type)
  self.simTypeData = TableDataBase.listSimCombatTypeDatas:GetDataById(self.simResourceData.sim_type)
end

function UISimCombatResourceStageDesc:RegisterEvent()
  function self.ItemUpdateHandler()
    self:ClearUpItems()
    
    self:SetActUpVisible()
  end
...
function UISimCombatResourceStageDesc:UnregisterEvent()
...
function UISimCombatResourceStageDesc:OnRelease(isDestroy)
  self.challengeSlotTable = nil
  self.ratingSlotTable = nil
  self.enemySlotTable = nil
  self.firstRewardSlotTab = nil
  self.rewardSlotTable = nil
  self.ui = nil
...
function UISimCombatResourceStageDesc:refreshChallengeItem(isRecover)
  ...
  if self.simEntranceData.id == enumId then
...
function UISimCombatResourceStageDesc:refreshEnemyItem()
  ...
  for i = 0, sorted.Count - 1 do
    ...
    UIUtils.GetButtonListener(slot.mBtn_OpenDetail.gameObject).onClick = function()
      CS.RoleInfoCtrlHelper.Instance:InitSysEnemyDataByStage(enemyId, self.stageData.stage_class, self.stageData.id)
    end
  end
end
...
function UISimCombatResourceStageDesc:refreshFirstRewardItem()
  ...
  if self.simEntranceData.id == enumId then
    ...
    local ratingEnumId = LuaUtils.EnumToInt(curRatingType)
    for i, cashGradeData in pairs(cashGradeDataList) do
      if ratingEnumId < cashGradeData.grade_id then
...
function UISimCombatResourceStageDesc:split(input, delimiter)
...
function UISimCombatResourceStageDesc:refreshRewardItem()
  ...
  local dropTable = {}
  if self.simCombatTypeCfg.drop_view_new == 1 then
    local dropView = self.simCombatCfg.normal_drop_view
    for itemId, countStr in pairs(dropView) do
      local countTab = self:split(countStr, "|")
      table.insert(dropTable, {
        ItemId = itemId,
        Count = tonumber(countTab[1]),
        MaxCount = tonumber(countTab[2])
      })
    end
  else
    local dropView = self.stageData.normal_drop_view_list
    for itemId, count in pairs(dropView) do
      table.insert(dropTable, {ItemId = itemId, Count = count})
    end
  end
  ...
  if self.simEntranceData.id == enumId then
    ...
    local cashGradeDataList = NetCmdStageRatingData:GetSortedCashGradeDataList(gradeGroupId)
    local curRatingType = NetCmdStageRatingData:GetCashRating(self.stageData.id)
    local curRatingTypeId = LuaUtils.EnumToInt(curRatingType)
    for i, cashGradeData in pairs(cashGradeDataList) do
      if curRatingTypeId >= cashGradeData.grade_id then
        for itemId, count in pairs(cashGradeData.AwardDropViewList) do
          table.insert(dropTable, {
            ItemId = itemId,
            Count = count,
  ...
  for i, dropInfo in ipairs(dropTable) do
    local itemId = dropInfo.ItemId
    local count = dropInfo.Count
    local maxCount = dropInfo.MaxCount
    local cashGradeData = dropInfo.CashGradeData
    local itemData = TableData.listItemDatas:GetDataById(itemId)
    ...
    elseif itemData.type == GlobalConfig.ItemType.EquipmentType then
      ...
      slot:SetItemData(itemId, count, nil, false, nil, nil)
    end
    if maxCount ~= nil then
      slot:SetMinAndMaxCount(count, maxCount)
    end
...
function UISimCombatResourceStageDesc:refreshCostItem()
  ...
  IconUtils.GetItemIconSpriteAsync(costItemId, self.ui.mImage_CostIcon)
  self.ui.mText_CostNum.text = CS.LuaUIUtils.GetMaxNumberText(staminaCost)
  self.ui.mText_CostNum.color = staminaCost > costItemNum and ColorUtils.RedColor or self.defaultCostColor
end
...
function UISimCombatResourceStageDesc:refreshGrpBtn()
  ...
  if self.simEntranceData.id == enumId then
    ...
    if cashGradeData then
      IconUtils.GetSimCombatTopRightSpriteAsync(cashGradeData.grade_id, self.ui.mImage_Rating)
    end
  end
...
function UISimCombatResourceStageDesc:SetActUpVisible()
  self.upItemList = {}
  self.hasUp = false
  local regressMax = NetCmdActivityRegressData:GetActivityUpCountMaxByStageType(self.simEntranceData.id)
  if 0 < regressMax then
    local regressCurrent = NetCmdActivityRegressData:GetActivityUpCountCurrent()
    local item = SimCombatResourceUpItem.New()
    item:InitCtrl(self.ui.mTrans_Up, self.ui.mTrans_UpParent)
    item:SetData(NetCmdActivityRegressData:GetActivityDesc(), regressCurrent, regressMax)
    table.insert(self.upItemList, item)
    self.hasUp = 0 < regressCurrent
  end
  ...
  for i = 0, dropUpActivities.Count - 1 do
    ...
    local current = NetCmdActivityDropUpData:GetActivityUpCurrentByStageAndActivity(self.simEntranceData.id, activityId)
    local enable, max = NetCmdActivityDropUpData:GetActivityUpMaxByStageAndActivity(self.simEntranceData.id, activityId)
    if enable then
      local item = SimCombatResourceUpItem.New()
      local reason = TableData.listUpActivityDatas:GetDataById(activityId).activity_text.str
      item:InitCtrl(self.ui.mTrans_Up, self.ui.mTrans_UpParent)
      item:SetData(reason, current, max)
      table.insert(self.upItemList, item)
      if false == self.hasUp then
        self.hasUp = 0 < current
      end
    end
  end
end

function UISimCombatResourceStageDesc:ClearUpItems()
  if self.upItemList == nil then
    return
  end
  for _, item in ipairs(self.upItemList) do
    item:OnRelease(true)
  end
  self.upItemList = nil
end
...
function UISimCombatResourceStageDesc:getGradeGroupId()
  if self.simResourceData.cash_group_id.Count ~= 2 then
    gferror("simResourceDataId" .. self.simResourceData.id .. "\231\154\132cash_group_id \232\167\163\230\158\144\233\148\153\232\175\175!")
    return 0
  end
