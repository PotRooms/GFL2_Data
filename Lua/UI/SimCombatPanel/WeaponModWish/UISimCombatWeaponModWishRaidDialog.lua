...
function UISimCombatWeaponModWishRaidDialog:StartRaid()
  ...
  if self.mData.WeaponModData then
    local defaultLimit = self.mData.WeaponModData.defaultLimit
    local limitCount = defaultLimit * self.curRaidTimes
    local upInfo = NetCmdActivityDropUpData:GetOneDropUpTimes(LuaUtils.EnumToInt(self.mData.WeaponModData.stageType))
    if 0 < upInfo.Count then
      local realCount = math.min(upInfo.Count, self.curRaidTimes)
      for i = 1, realCount do
        local up = upInfo[i - 1]
        limitCount = limitCount + math.ceil(up * defaultLimit)
      end
    end
    if TipsManager.CheckItemIsOverflowAndStopByList(self.mData.WeaponModData.reward, limitCount) then
      return
    end
  end
  local sendRaidCmd = function()
    self.ui.mBtn_Confirm.interactable = false
    NetCmdSimulateBattleData:SendSimCombatWeaponModAssignedDrop(self.stageData.id, self.stageData.type, self.wishSelectItem.selectSuitID, function(code)
...
function UISimCombatWeaponModWishRaidDialog:OnUpdateCost()
  self.currentCostItemCount = NetCmdItemData:GetNetItemCount(self.mData.costItemId)
  self:refreshCostText()
end
