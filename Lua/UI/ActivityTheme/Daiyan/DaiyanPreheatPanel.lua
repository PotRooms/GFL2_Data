  ...
  if data == nil then
    NetCmdRecentActivityData:ReqPlanActivityData(PlanType.PlanFunctionActivityThematic, function(ret)
      if ret == ErrorCodeSuc then
        ...
        NetCmdThemeData:SendThemeCheckInQuestionInfo(self.activityEntranceData.id, function(ret)
          self:UpdateInfo()
          self:UpdateScoreInfo()
          self:UpdateQuestInfo()
          self:UpdateProcess()
...
function DaiyanPreheatPanel:UpdateInfo()
  ...
  CS.UIUtils.GetAndSetActivityHintText(self.mUIRoot, self.activityConfigData.id, 2, self.activityModuleData.id, 101)
  ...
  self.ui.mImg_Avatar.sprite = IconUtils.GetCharacterHeadFullName(self.activityModuleData.activity_role)
  self.ui.mImg_Bg.sprite = IconUtils.GetUIResSprite("ActivityTheme/Daiyan/" .. self.activityModuleData.activity_main_bg)
...
function DaiyanPreheatPanel:UpdateScoreInfo()
  ...
  if self.scoreState == 0 then
    self.ui.mText_Score.text = self.scoreData.desc.str .. string_format(TableData.GetHintById(270143), 0, 1)
    setactive(self.ui.mTrans_ScoreFinished.gameObject, false)
    setactive(self.ui.mBtn_ScoreBtn.gameObject, true)
  else
    self.ui.mText_Score.text = self.scoreData.desc.str .. "<color=#dcc173>" .. string_format(TableData.GetHintById(270143), 1, 1) .. "</color>"
...
function DaiyanPreheatPanel:UpdateQuestInfo()
  ...
  if self.questState == 0 then
    self.ui.mText_Quest.text = self.questData.desc.str .. string_format(TableData.GetHintById(270143), 0, 1)
    ...
    self.ui.mText_Quest.text = self.questData.desc.str .. "<color=#dcc173>" .. string_format(TableData.GetHintById(270143), 1, 1) .. "</color>"
  end
  self:UpdateProcess()
end

function DaiyanPreheatPanel:UpdateProcess()
  local curValue = NetCmdThemeData:GetTotalPoint(self.activityEntranceData.id)
  ...
  if rewardIDList then
    for i = 1, rewardIDList.Count do
      ...
      if data and cell then
        ...
        local state = NetCmdThemeData:GetPhaseRewardState(self.activityEntranceData.id, rewardIDList[i - 1])
    ...
    elseif curValue <= pointList[2] then
      ...
      process = offSetList[2] + (curValue - pointList[2]) / (pointList[3] - pointList[2]) * 212 / 426
    end
