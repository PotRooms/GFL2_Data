...
function UIDormChrBehaviourPanel:OnShowStart()
  ...
  local currRandomAnimId = NetCmdLoungeData:GetRandomAnimId()
...
function UIDormChrBehaviourPanel:SetBaseData()
  ...
  local idList = {}
  local ids = TableData.listDormFormationByModleIdDatas:GetDataById(gunModelID, true)
  if ids ~= nil then
    local d = ids.Id
    local count = d.Count
    for i = 0, count - 1 do
...
function UIDormChrBehaviourPanel:RightBtnClickFunction(item)
  ...
  UISystem.UISystemBlackCanvas:PlayFadeOutEnhanceBlack(0.4, function()
    TimerSys:DelayCall(0.4, function()
      ...
      NetCmdLoungeData:SetCurrSelectAnim(item.behaviourData.id)
