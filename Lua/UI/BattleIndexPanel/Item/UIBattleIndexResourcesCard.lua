...
function UIBattleIndexResourcesCard:InitCtrl(parent, template)
  ...
  self:InitCtrlWithoutInstance(go)
end

function UIBattleIndexResourcesCard:InitCtrlWithoutInstance(insObj)
  ...
  UIUtils.AddBtnClickListener(self.ui.mBtn_GrpDropup.gameObject, function()
    UIManager.OpenUIByParam(UIDef.UIComDropUpDialog, {
      stageType = self.simCombatEntranceData.id
    })
  end)
end

function UIBattleIndexResourcesCard:RegisterEvent()
...
function UIBattleIndexResourcesCard:UnregisterEvent()
...
function UIBattleIndexResourcesCard:refreshTimes()
  ...
  local timerData
  if simTypeData.extra_drop_timer ~= 0 then
    timerData = TableData.listTimerDatas:GetDataById(simTypeData.extra_drop_timer)
  end
  local time
...
function UIBattleIndexResourcesCard:RefreshUpCount()
  local totalMax = 0
  local totalCurrent = 0
  local regressMax = NetCmdActivityRegressData:GetActivityUpCountMaxByStageType(self.simCombatEntranceData.id)
  ...
  local enable, dropUpMax = NetCmdActivityDropUpData:GetActivityUpCountMaxByStageType(self.simCombatEntranceData.id)
  if enable then
    totalMax = totalMax + dropUpMax
    totalCurrent = totalCurrent + NetCmdActivityDropUpData:GetActivityUpCountCurrentByStageType(self.simCombatEntranceData.id)
  end
  setactive(self.ui.mBtn_GrpDropup.gameObject, 0 < totalMax or enable)
  self.ui.mText_DropupTimes.text = totalCurrent .. "/" .. totalMax
  local spriteName = 0 < totalCurrent and "Icon_DropupL" or "Icon_Dropup_GreyL"
...
function UIBattleIndexResourcesCard:ChallengeTime()
  ...
  local timerData
  if entranceData.item_times ~= 0 then
...
function UIBattleIndexResourcesCard:onClickSelf()
  if self.onClickCallback then
    self.onClickCallback(self.simCombatEntranceData, self.index)
  end
end
