...
function UIBattleIndexModeListItem:SetData(data)
  ...
  if self.mData.id == 2 then
    ...
    local uniteRedDot = 0 < NetCmdSimulateBattleData:GetUniteRewardRedDot()
    local uniteRefreshRedDot = 0 < NetCmdSimulateBattleData:GetUniteRefreshRedDot()
    setactive(self.ui.mTrans_RedPoint, not self.mIsLock and NetCmdPVPData:CheckPvpRedPoint() ~= 0 or mythicRedPoint or hasReward or simBattleRedPoint or systemHasLook == false or uniteRedDot == true or uniteRefreshRedDot == true)
  ...
  elseif self.mData.id == 5 then
    ...
    for i = 0, self.mData.detail_id.Count - 1 do
      ...
      if chapterData then
        ...
        for j = 0, chapterData.unlock.Count - 1 do
          if not AccountNetCmdHandler:CheckSystemIsUnLock(chapterData.unlock[j]) then
    ...
    if lockCount >= self.mData.detail_id.Count or 0 < lockAchieveCount then
      self.mIsLock = true
    end
