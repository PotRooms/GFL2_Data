...
function ChrWeaponItemV4:SetWeaponData(gunCmdData, weaponCmdData, callback, needRedPoint)
  ...
  setactive(self.ui.mTrans_WeaponSkinIcon.gameObject, self.weaponCmdData.UseSkin)
  setactive(self.ui.mTrans_Recommend.gameObject, self.weaponCmdData.IsRecommendedWeapon)
...
function ChrWeaponItemV4:UpdateRedPoint()
  if self.mGunCmdData ~= nil then
    local redPoint = 0
    if self.weaponCmdData.Rank > self.mGunCmdData.WeaponData.Rank and not self.weaponCmdData.IsEquipped and self.weaponCmdData:HasCanChangeWeaponRedPointPref() then
      redPoint = 1
    end
