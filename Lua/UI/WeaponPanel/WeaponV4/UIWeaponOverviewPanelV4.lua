...
function UIWeaponOverviewPanelV4:OnInit(data)
  ...
  setactive(self.ui.mBtn_SpecialEffect.gameObject, false)
  UIUtils.GetButtonListener(self.ui.mBtn_SpecialEffect.gameObject).onClick = function()
    self:OnClickSpecialEffect()
  end
  UIUtils.GetButtonListener(self.ui.mBtn_Specific.gameObject).onClick = function()
    self:OnClickSpecific()
  end
  ...
  UIUtils.GetButtonListener(self.ui.mBtn_WeaponRecommend.gameObject).onClick = function()
    self:OnClickWeaponRecommend()
  end
...
function UIWeaponOverviewPanelV4:OnClose()
  ...
  self.curClickWeaponItemId = 0
  if self.gunCmdData ~= nil and self.weaponReplaceList ~= nil then
    self.gunCmdData:ResetRecommendedWeapon(self.weaponReplaceList)
  end
end
...
function UIWeaponOverviewPanelV4:ItemRenderer(index, renderData)
  ...
  item:SetWeaponData(self.gunCmdData, data, function(tempItem)
    self:OnClickWeaponItem(tempItem)
    data:SetCanChangeWeaponRedPointPref()
    item:UpdateRedPoint()
  end)
...
function UIWeaponOverviewPanelV4:SetWeaponData()
  ...
  if self.mIsRelatedBP or self.uiChrWeaponPanelV4.openFromType == UIWeaponPanel.OpenFromType.GachaPreview then
    setactive(self.ui.mBtn_Root.gameObject, false)
  ...
  setactive(self.ui.mToggle_Contrast.gameObject, false)
  if not self.isCompose then
    self:UpdateRedPoint()
    if self.uiChrWeaponPanelV4.needReplaceBtn then
  ...
  self:UpdateWeaponSpecificSkill()
  self:UpdateAction()
  self:UpdateSkill()
  self:UpdateAttribute()
  self:UpdateBpType()
  self:UpdateWeaponSkinBtnState()
end

function UIWeaponOverviewPanelV4:SetTopData()
  ...
  setactive(self.ui.mTrans_WeaponSkinIcon, self.curSelectWeaponCmdData.UseSkin)
  ...
  setactive(self.ui.mText_WeaponQuality.gameObject, false)
  setactive(self.ui.mText_WeaponWeakpoint.gameObject, true)
  self.ui.mText_WeaponWeakpoint.text = self.curSelectWeaponCmdData.WeaponTagData.name.str
  ...
  if self.curSelectWeaponCmdData.BreakTimes ~= 0 then
    ...
    UIWeaponGlobal:SetBreakTimesColor(self.ui.mImg_Pic, self.curSelectWeaponCmdData.BreakTimes, self.curSelectWeaponCmdData.MaxBreakTime, false)
  else
    UIWeaponGlobal.SetBreakTimesImg(self.ui.mImg_Num, 1, self.curSelectWeaponCmdData.MaxBreakTime)
    UIWeaponGlobal:SetBreakTimesColor(self.ui.mImg_Pic, 1, self.curSelectWeaponCmdData.MaxBreakTime, false)
  end
...
function UIWeaponOverviewPanelV4:UpdateRedPoint()
  ...
  if self.curSelectWeaponCmdData ~= nil then
    ...
    local weaponSkinRedPoint = self.curSelectWeaponCmdData:GetWeaponSkinRedPoint()
    setactive(self.ui.mObj_WeaponSkinRedPoint.transform.parent.gameObject, 0 < weaponSkinRedPoint)
  end
end
...
function UIWeaponOverviewPanelV4:UpdateWeaponSpecificSkill()
  local weaponSkillDatas = self.curSelectWeaponCmdData.WeaponSkillDatas
  local hasWeaponSpecificSkill = weaponSkillDatas ~= nil and weaponSkillDatas.Count ~= 0
  setactive(self.ui.mBtn_Specific.gameObject, hasWeaponSpecificSkill)
  self.ui.mBtn_Specific.enabled = hasWeaponSpecificSkill
end
...
function UIWeaponOverviewPanelV4:UpdateSkill()
  ...
  if data then
    ...
    local skillDes = self.curSelectWeaponCmdData:FormatSkillDesc(data.id)
    self.ui.mTextFit_Describe.text = skillDes
...
function UIWeaponOverviewPanelV4:UpdateAttribute()
  ...
  for i = 0, expandList.Count - 1 do
    ...
    local value = tmpWeaponCmdData:GetWeaponValueByName(lanData.sys_name, false)
    if 0 < value then
      local attr = {}
  ...
  for i = 1, 5 do
    ...
    if i < tmpAttrParent.childCount then
      item:InitCtrl(tmpAttrParent.gameObject, tmpAttrParent:GetChild(i))
    else
  ...
  setactive(self.ui.mBtn_Detail.gameObject, false)
end

function UIWeaponOverviewPanelV4:UpdateAction()
  local canShow = not self.mIsRelatedBP and self.uiChrWeaponPanelV4.openFromType ~= UIWeaponPanel.OpenFromType.GachaPreview
  if not canShow then
    return
  end
  if self.gunCmdData == nil then
    setactive(self.ui.mTrans_Equiped.gameObject, false)
    setactive(self.ui.mBtn_BtnReplace.gameObject, false)
  else
    ...
    setactive(self.ui.mBtn_BtnReplace.gameObject, not isCurWeapon and not self.mIsRelatedBP)
  end
  ...
  setactive(self.ui.mBtn_BtnReplace.transform.parent, not self.mIsRelatedBP)
  setactive(self.ui.mBtn_Root.gameObject, self.uiChrWeaponPanelV4.needReplaceBtn)
  self.uiChrWeaponPanelV4:ActiveChrChangeBtn(self.uiChrWeaponPanelV4.needReplaceBtn and not self.isShowReplaceList)
  setactive(self.uiChrWeaponPanelV4.ui.mTrans_Arrow.gameObject, self.uiChrWeaponPanelV4.needReplaceBtn and not self.isShowReplaceList or self.uiChrWeaponPanelV4.tmpUIWeaponPanelList ~= nil)
  if self.isCompose then
    return
  end
  local canBreak = not self.curSelectWeaponCmdData.IsReachMaxBreak
  local canLevelUp = not self.curSelectWeaponCmdData.IsReachMaxLv
  self.ui.mBtn_Break.interactable = canBreak
  self.ui.mBtn_PowerUp.interactable = canLevelUp
  if canBreak or canLevelUp then
    local unlockWeaponUpgrade = AccountNetCmdHandler:CheckSystemIsUnLock(SystemList.WeaponUpgrade)
    local unlockWeaponClass = AccountNetCmdHandler:CheckSystemIsUnLock(SystemList.WeaponClass)
    setactive(self.ui.mBtn_Break.transform.parent.gameObject, unlockWeaponClass)
    setactive(self.ui.mBtn_PowerUp.transform.parent.gameObject, unlockWeaponUpgrade)
    setactive(self.ui.mTrans_CultivateFullLv.gameObject, false)
  else
    setactive(self.ui.mBtn_Break.transform.parent.gameObject, false)
    setactive(self.ui.mBtn_PowerUp.transform.parent.gameObject, false)
    setactive(self.ui.mTrans_CultivateFullLv.gameObject, true)
  end
end
...
function UIWeaponOverviewPanelV4:UpdateReplaceList()
  ...
  if not self.isShowReplaceList then
    return
  end
  self.weaponReplaceList = self.gunCmdData:GetWeaponReplaceList(self.weaponReplaceList, self.weaponCmdData.id)
...
function UIWeaponOverviewPanelV4:UpdateWeaponSkinBtnState()
  local openWeaponSkin = OpenFunctionsManager:CheckFunctionOpen(100001) == 1
  setactive(self.ui.mBtn_WeaponSkinChange.gameObject, not self.isCompose and self.uiChrWeaponPanelV4.openFromType ~= UIWeaponPanel.OpenFromType.GachaPreview and openWeaponSkin)
  if not self.isCompose then
    local unlock = AccountNetCmdHandler:CheckSystemIsUnLock(SystemList.WeaponSkin)
    self.ui.mAnimator_WeaponSkinChange:SetBool("UnLock", unlock)
    UIUtils.GetButtonListener(self.ui.mBtn_WeaponSkinChange.gameObject).onClick = function()
      if TipsManager.NeedLockTips(SystemList.WeaponSkin) then
        return
      end
      self.uiChrWeaponPanelV4:OnClickWeaponSkin(true)
    end
  end
end

function UIWeaponOverviewPanelV4:RefreshDataByWeaponCmdData(weaponCmdData)
...
function UIWeaponOverviewPanelV4:OnClickExclusive()
  ...
  local skillDes = self.curSelectWeaponCmdData:FormatSkillDesc(battleSkillDisplayData.id)
  local data = {
    [1] = weaponPrivateName,
    [2] = skillDes
  }
...
function UIWeaponOverviewPanelV4:OnClickSpecific()
  self.ui.mBtn_Specific.enabled = false
  local param = {
...
function UIWeaponOverviewPanelV4:OnClickReplace(boolean)
  ...
  if boolean then
    ...
    UIBarrackWeaponModelManager:SwitchView(1, true)
  else
...
function UIWeaponOverviewPanelV4:OnClickPowerUpBtn(targetContent)
  ...
  if targetContent == 1 then
    local param = CS.UIChrWeaponBreakWindowDialog.UIParam()
    param.WeaponCmdData = self.weaponCmdData
    param.GunCmdData = self.gunCmdData
    UIManager.OpenUIByParam(enumUIPanel.UIChrWeaponBreakWindowDialog, param)
  else
    local uiChrWeaponPanelV4ExpTable = CS.UIChrWeaponLevelUpWindowDialog.UIWeaponLevelExpTableParam()
    uiChrWeaponPanelV4ExpTable.mImg_ProgressBarBefore = self.uiChrWeaponPanelV4.ui.mImg_ProgressBarBefore
    uiChrWeaponPanelV4ExpTable.mImg_ProgressBarAfter = self.uiChrWeaponPanelV4.ui.mImg_ProgressBarAfter
    uiChrWeaponPanelV4ExpTable.mTrans_ExpBar = self.uiChrWeaponPanelV4.ui.mTrans_ExpBar
    local param = CS.UIChrWeaponLevelUpWindowDialog.UIParam()
    param.WeaponCmdData = self.weaponCmdData
    param.GunCmdData = self.gunCmdData
    param.uiChrWeaponPanelV4ExpTable = uiChrWeaponPanelV4ExpTable
    UIManager.OpenUIByParam(enumUIPanel.UIChrWeaponLevelUpWindowDialog, param)
  end
end
...
function UIWeaponOverviewPanelV4:OnClickConsume()
  TipsPanelHelper.OpenUITipsPanel(self.itemData, 0, true)
end
...
function UIWeaponOverviewPanelV4:OnClickWeaponRecommend()
  UIManager.OpenUIByParam(CS.GF2.UI.enumUIPanel.UIChrRecommendDialog, LuaUtils.GetChrRecommendDialogParam(self.gunCmdData.Id))
end
