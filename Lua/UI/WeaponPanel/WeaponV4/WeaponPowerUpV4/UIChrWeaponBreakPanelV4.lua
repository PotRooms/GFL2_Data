...
function UIChrWeaponBreakPanelV4:OnInit(data)
  ...
  UIUtils.GetButtonListener(self.ui.mBtn_GrpWeaponConsume.gameObject).onClick = function()
    ...
    TipsPanelHelper.OpenUITipsPanel(self.curCostItemData, 0, true, false, id)
  end
...
function UIChrWeaponBreakPanelV4:UpdateSkill()
  ...
  if skillData then
    ...
    local skillDes = CS.GF2.Battle.SkillUtils.GetSkillDes(skillData.id, false, 0, nil)
    self.ui.mTextFit_Describe1.text = skillDes
  end
end
...
function UIChrWeaponBreakPanelV4:SendLevelUp()
  ...
  if self.uiChrWeaponPowerUpPanelV4 ~= nil then
    self.uiChrWeaponPowerUpPanelV4:ShowMask(true)
  end
  NetCmdWeaponData:SendGunWeaponBreak(self.weaponCmdData.id, self.curCostWeaponCmdData.id, function(ret)
    if self.uiChrWeaponPowerUpPanelV4 ~= nil then
      self.uiChrWeaponPowerUpPanelV4:ShowMask(false)
    end
