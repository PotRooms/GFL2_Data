...
UIWeaponGlobal.WeaponContentTypeV4 = {
  Weapon = 1,
  WeaponPart = 2,
  WeaponSkin = 3
}
...
UIWeaponGlobal.WeaponBreakColor = {
  ...
  OrangeColor = ColorUtils.StringToColor("F26C1C")
}
...
function UIWeaponGlobal.SetBreakTimesImg(img, num, maxNum, isSmall)
  ...
  UIWeaponGlobal:SetBreakTimesColor(img, num, maxNum)
end

function UIWeaponGlobal:SetBreakTimesColor(img, num, maxNum, withAlpha)
  if withAlpha == nil then
    withAlpha = true
  end
  if maxNum <= num then
    local a = img.color.a
...
function UIWeaponGlobal.SetIsReadyToStartTutorial(boolean)
  gfinfo("[Tutorial] UIWeaponGlobal.isReadyToStartTutorial" .. tostring(boolean))
