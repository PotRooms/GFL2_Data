...
function ActivityMusePanel:OnAwake(root, data)
  ...
  setactive(self.ui.mTrans_Collect.gameObject, true)
  setactive(self.ui.mTrans_Exchange.gameObject, false)
  self:AddBtnListener()
  self:ManualUI()
end
...
function ActivityMusePanel:ManualUI()
  setactive(self.ui.mBtn_Hint.gameObject, false)
  setactive(self.ui.mTrans_TabModeList.gameObject, false)
  self.activityID = 101
  ...
  for i = 1, self.ui.mTrans_Item1.childCount do
    ...
    UIUtils.GetButtonListener(btn.gameObject).onClick = function()
      TipsPanelHelper.OpenUITipsPanel(TableData.GetItemData(self.collectDataList[index].id))
    end
...
function ActivityMusePanel:UpdateMySubList()
  ...
  local count
  if self.mysubList.Count >= self.museData.order_num then
