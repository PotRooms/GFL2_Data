...
function UIGachaInfoItem:SetNormalItem(instObj, itemData, isChr)
  ...
  if isChr then
    ...
    local iconDuty = instObj:Find("Trans_GrpChr/Img_Duty"):GetComponent(typeof(CS.UnityEngine.UI.Image))
    icon.sprite = ResSys:GetAtlasSprite("Icon/Avatar/Avatar_Bust_" .. itemData.code)
    local gunData = TableData.listGunDatas:GetDataById(itemData.id)
    local dutyData = TableData.listGunDutyDatas:GetDataById(gunData.duty)
    iconDuty.sprite = IconUtils.GetGunTypeIcon(dutyData.icon)
...
function UIGachaInfoItem:SetData(type, data)
  ...
  elseif type == UIGachaInfoItem.ItemInfoType.UpRate then
    setactive(self.mTrans_Title, data.rank == 5)
    self.gachaId = data.gachaId
    self.mText_Title.text = TableData.GetHintById(107059)
