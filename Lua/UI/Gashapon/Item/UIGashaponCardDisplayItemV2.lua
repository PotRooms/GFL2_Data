...
function UIGashaponCardDisplayItemV2:InitCtrl(instObj, index)
  self.index = index
  self:SetRoot(instObj.transform)
  self.ui = {}
  self:LuaUIBindTable(self.mUIRoot, self.ui)
  self:__InitCtrl()
  setactive(self.ui.mImg_Character, false)
  setactive(self.ui.mImg_Weapon, false)
  ...
  UIUtils.GetButtonListener(self.mBtn_self.gameObject).onClick = function()
    if self.mStcData.type == GlobalConfig.ItemType.GunType then
      CS.RoleInfoCtrlHelper.Instance:InitSysPlayerDataById(self.mData.ItemId)
    else
      TipsPanelHelper.OpenUITipsPanel(self.mStcData, 0, false)
    end
  end
end
...
function UIGashaponCardDisplayItemV2:InitItemInfo()
  ...
  if self.mStcData.type == GlobalConfig.ItemType.GunType then
    ...
    IconUtils.GetAtlasSpriteAsyc("Icon/Avatar/Avatar_Gacha_" .. gunData.code, function(s, o, arg)
      if o then
        self.ui.mImg_Character.sprite = o
      else
        self.ui.mImg_Character.sprite = IconUtils.GetCharacterGachaSprite(gunData.code)
      end
      setactive(self.ui.mImg_Character, true)
    end)
  elseif self.mStcData.type == GlobalConfig.ItemType.Weapon then
    IconUtils.GetAtlasSpriteAsyc("Icon/Weapon/" .. icon .. "_1024", function(s, o, arg)
      if o then
        self.ui.mImg_Weapon.sprite = o
      else
        self.ui.mImg_Weapon.sprite = IconUtils.GetWeaponNormalSprite(icon)
      end
      setactive(self.ui.mImg_Weapon, true)
    end)
  else
