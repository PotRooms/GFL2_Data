...
function UIGachaShoppingDetailPanel:OnInit(root, data)
  ...
  function self.onRequestGachaRecord(msg)
    if msg.Sender == nil then
      self.curPage = 1
      self.maxPage = 1
    else
      self.curPage = math.max(1, msg.Sender)
      if msg.Sender == 0 then
        self.maxPage = 0
      end
      self.maxPage = math.max(self.maxPage or 0, self.curPage)
      self.nextPage = msg.Content
    end
    self:UpdateHistoryPage()
    self.canRequest = true
  end
  
  MessageSys:AddListener(UIEvent.RequestGachaRecord, self.onRequestGachaRecord)
end

function UIGachaShoppingDetailPanel:SwitchPage(value)
  if value == -1 then
  ...
  elseif self.canRequest then
    ...
    GashaponNetCmdHandler:RequestRecordData(self.curPage + 1, self.gachaData.type, self.nextPage)
  end
end

function UIGachaShoppingDetailPanel:UpdateHistoryPage()
  self.ui.mText_Page.text = tostring(self.curPage)
  self.ui.mBtn_Left.interactable = self.curPage > 1
  self.ui.mBtn_Right.interactable = self.curPage < self.maxPage or self.nextPage ~= ""
...
function UIGachaShoppingDetailPanel:InitData(data)
  ...
  GashaponNetCmdHandler:RequestRecordData(0, self.gachaData.type)
end
...
function UIGachaShoppingDetailPanel:InitHistoryData()
  self.gachaHistory = GashaponNetCmdHandler:GetGachaHistory(self.curPage)
...
function UIGachaShoppingDetailPanel:OnClickTab(gameObj, idValue)
  ...
  if idValue == 3 then
    ...
    local titleData = {}
    titleData.str = "\229\175\185\232\177\161\230\184\133\229\141\149"
...
function UIGachaShoppingDetailPanel:OnClose()
  MessageSys:RemoveListener(UIEvent.RequestGachaRecord, self.onRequestGachaRecord)
  self.mView = nil
  self.curTabId = nil
  for i = 1, #self.tabList do
