...
function UIGachaMainPanelV2:OnInit(root, data)
  ...
  function self.gachaGetSSR()
    self.isGetSSR = true
  end
  
  function self.onReLoginSuccess()
    self.mIsDrawClicked = false
  end
  
  MessageSys:AddListener(UIEvent.ReLoginSuccess, self.onReLoginSuccess)
  MessageSys:AddListener(UIEvent.GachaGetSSR, self.gachaGetSSR)
...
function UIGachaMainPanelV2:OnGachaSceneLoadFinish()
  ...
  if self.isHide == nil or not self.isHide then
...
function UIGachaMainPanelV2:OnBackFrom()
  ...
  if not self.mTabDisplayItemList[self.curTab].mEventData.IsOpen then
    ...
    setactive(self.ui.mTrans_PreviewRedpoint, false)
  end
...
function UIGachaMainPanelV2.OneFingerDragingEventHandle(touchPoint, offset)
  if UIGachaMainPanelV2.gachaScene == nil then
    UIGachaMainPanelV2.gachaScene = SceneSys:GetGachaScene()
    return
  end
...
function UIGachaMainPanelV2:OnShop()
  UISystem:JumpByID(5003)
end

function UIGachaMainPanelV2:OnShopClicked(obj)
  UIManager.OpenUIByParam(CS.GF2.UI.enumUIPanel.UIGachaProbabilityDetailsDialog, self.mCurGachaData)
end
...
function UIGachaMainPanelV2:OnActivityTabClicked(index, playAnim)
  ...
  GashaponNetCmdHandler:PoolTagClick(item.mEventData.GachaID)
  self:RefreshText()
  local info = CS.OssTabSwitchInfo(UIDef.UIGashaponMainPanel, 0, 0, item.mEventData.GachaID)
...
function UIGachaMainPanelV2:SetEventData(data)
  ...
  if data.GunUpCharacter == 0 then
    ...
    self.upSwitchTimer = TimerSys:DelayCall(0.2, function()
      ...
      local dutyData = CS.GunCmdData.GetGunDutyDataByGunId(data.GunUpCharacter)
      self.ui.mImg_Duty.sprite = IconUtils.GetGunTypeIcon(dutyData.icon .. "_W")
      self.ui.mText_DutyName.text = dutyData.name.str
      local secondDutyText = CS.GunCmdData.GetGunSecondDutyStrByGunId(data.GunUpCharacter)
  ...
  setactive(self.ui.mTrans_GrpNum, data.Type == 1 and not GashaponNetCmdHandler.GachaOptionalFlag)
...
function UIGachaMainPanelV2:RefreshText()
  ...
  if self.mCurGachaData.Type ~= 5 then
    txtTen.text = "\195\15110"
  else
    self.ui.mText_TenNumNew.text = "\195\1518"
  end
  imgOne.sprite = IconUtils.GetItemIconSprite(ticketItem.id)
  txtOne.text = "\195\1511"
  ...
  if count < tenNum then
    if self.mCurGachaData.GunUpCharacter == 0 then
      self.ui.mText_TenNumNew.color = ColorUtils.RedColor
...
function UIGachaMainPanelV2:OnFadeInFinish()
  self.onFadeInFinish = true
end

function UIGachaMainPanelV2:OnShowFinish()
  local isTutorial = TutorialSystem.IsInTutorial
  if isTutorial then
    TimerSys:DelayFrameCall(1, function()
      local contentSize = LuaUtils.GetRectTransformSize(self.ui.mContent_LeftTabMobile.gameObject)
      local parentSize = LuaUtils.GetRectTransformSize(self.ui.mContent_LeftTabMobile.transform.parent.gameObject)
      if contentSize.y > parentSize.y then
        self.ui.mContent_LeftTabMobile.transform.localPosition = Vector3(self.ui.mContent_LeftTabMobile.transform.localPosition.x, self.ui.mContent_LeftTabMobile.transform.localPosition.y + contentSize.y - parentSize.y, 0)
      end
    end)
  end
end
...
function UIGachaMainPanelV2:OnOneTimeClicked(gameobj, isSuccess)
  local cachedMailList = NetCmdMailData:GetSortedMailList()
  if cachedMailList.Count + 1 > TableData.GlobalConfigData.MailMaxlimit then
    local limitmsg = TableData.GetHintById(107075)
    CS.PopupMessageManager.PopupString(limitmsg)
    self.mIsDrawClicked = false
    return
  end
  if self.onFadeInFinish ~= nil and not self.onFadeInFinish then
    return
  end
  ...
  if limitType ~= -1 and true == limitVisible then
    ...
    elseif cost > ticket then
      ...
      local num = cost
      local ticketItem = TableData.GetItemData(self.mCurGachaData.CostItemID)
      CS.QuickStorePurchase.QuickPurchase(ticketItem.Goodsid, 1, 107012, self.mCurGachaData.CostItemID, function(ret)
...
function UIGachaMainPanelV2:OnTenTimeClicked(gameobj, isSuccess)
  local cachedMailList = NetCmdMailData:GetSortedMailList()
  if cachedMailList.Count + 10 > TableData.GlobalConfigData.MailMaxlimit then
    local limitmsg = TableData.GetHintById(107075)
    CS.PopupMessageManager.PopupString(limitmsg)
    self.mIsDrawClicked = false
    return
  ...
  if limitType ~= -1 and true == limitVisible then
    ...
    elseif ticket < cost then
      ...
      local ticketItem = TableData.GetItemData(self.mCurGachaData.CostItemID)
      self.mIsDrawClicked = true
      CS.QuickStorePurchase.QuickPurchase(ticketItem.Goodsid, num, 107012, self.mCurGachaData.CostItemID, function(ret)
...
function UIGachaMainPanelV2:OnBuyTicketCallBack_1(ret)
  if ret == ErrorCodeSuc then
    ...
    gfdebug("\232\180\173\228\185\176\230\137\173\232\155\139\229\136\184\229\164\177\232\180\165")
...
function UIGachaMainPanelV2:OnTop(data)
  setactive(self.ui.mTrans_GrpNum, self.mCurGachaData.Type == 1 and not GashaponNetCmdHandler.GachaOptionalFlag)
  SceneSys:SwitchVisible(EnumSceneType.Gacha)
end
...
function UIGachaMainPanelV2:OnClose()
  MessageSys:RemoveListener(UIEvent.ReLoginSuccess, self.onReLoginSuccess)
  MessageSys:RemoveListener(UIEvent.GachaGetSSR, self.gachaGetSSR)
  ...
  self.gachaScene = nil
  SceneSys:UnLoadScene(EnumSceneType.Gacha)
end
