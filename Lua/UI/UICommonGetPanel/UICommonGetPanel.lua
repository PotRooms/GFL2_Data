...
function UICommonGetPanel:Close()
  if UICommonGetPanel.mView.mTrans_GrpPriceDetails.gameObject.activeSelf then
    UICommonGetPanel:HidePriceDetails()
  end
...
function UICommonGetPanel:OnSelectItem(gameObject)
  ...
  local num1 = string.format("%g", content.data.storeData.price)
  ...
  if content.data.storeData.IsMultiPrice then
    ...
    self.mView.mTxt_PriceSetailsNum.text = string.format("%g", content.data.storeData.price)
...
function UICommonGetPanel:OnBuyDataButtonClick(content)
  ...
  if self:NeedShowBuyTips(item, type) then
    return
  elseif self:StaminaOverFlowWarning(item.prizeData.num) then
    return
  else
...
function UICommonGetPanel:TakeQuestRewardCallBack()
  ...
  local hint = TableData.GetHintById(106305)
...
function UICommonGetPanel:NeedShowBuyTips(item, type)
  ...
  elseif error == UICommonGetPanel.ErrorType.ItemNotEnough then
    ...
    elseif type == UICommonGetPanel.GetType.Diamond then
      MessageBoxPanel.ShowItemNotEnoughMessage(item.itemData.id, function()
        self:OnDiamandNeedGotoStoreClicked()
      end)
    end
    return true
...
function UICommonGetPanel:OnBackFrom()
  self:UpdateView()
end

function UICommonGetPanel:OnTop()
  self:UpdateView()
end
