...
function ActivityTourTaskInfo:InitCtrl(ui)
  ...
  self.isOnlyShowRoundInfo = false
  self:RefreshAll()
end
...
function ActivityTourTaskInfo:Show(isShow)
  ...
  if not self.isOnlyShowRoundInfo then
    UIUtils.AnimatorFadeInOut(self.ui.mAnimator_TaskRoot, isShow)
  end
end

function ActivityTourTaskInfo:OnlyShowRoundInfo(isOnlyShowRoundInfo, ifForceRefresh)
  ifForceRefresh = ifForceRefresh or false
  if self.isOnlyShowRoundInfo == isOnlyShowRoundInfo and not ifForceRefresh then
    return
  end
  self.isOnlyShowRoundInfo = isOnlyShowRoundInfo
  self.ui.mAnimator_TaskShow:SetBool("Bool", self.isOnlyShowRoundInfo)
  UIUtils.AnimatorFadeInOut(self.ui.mAnimator_TaskRoot, not self.isOnlyShowRoundInfo)
end
...
function ActivityTourTaskInfo:RefreshRoundInfo()
  ...
  self.ui.mText_TaskProgress.text = UIUtils.StringFormat(MonopolyUtil:GetMonopolyActivityHint(270428), NetCmdMonopolyData.currentRound, MonopolyWorld.MpData.levelData.max_round)
end
