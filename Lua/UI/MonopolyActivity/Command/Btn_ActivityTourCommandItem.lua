...
function Btn_ActivityTourCommandItem:RefreshCommand()
  ...
  local minMove, maxMove = ActivityTourGlobal.GetOrderMoveRange(self.data)
  if minMove == maxMove then
    self.ui.mText_Step.text = tostring(minMove)
  else
    self.ui.mText_Step.text = UIUtils.StringFormat(MonopolyUtil:GetMonopolyActivityHint(270164), minMove, maxMove)
  end
end
