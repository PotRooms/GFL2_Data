...
function ActivityTourPointRandomItem:Refresh(minPoint, maxPoint, baseRoll, finalRoll, buffTrigger, showResult)
  for i = 6, 1, -1 do
  ...
  local bufPoint = finalRoll - baseRoll
  local listBuf = {}
  if buffTrigger then
    for i = 0, buffTrigger.Count - 1 do
      table.insert(listBuf, buffTrigger[i])
    end
  end
  self.bufPoint = bufPoint
  self.result = baseRoll
  ...
  if 0 < #listBuf then
    ...
    local buffData = TableData.listMonopolyEffectDatas:GetDataById(listBuf[1])
    if buffData then
...
function ActivityTourPointRandomItem:PlayTweenAin(minPoint, maxPoint)
  ...
  local setter1 = function(tempSelf, value)
    ...
    if self.stage1AudioPlay[index] == nil then
      AudioUtils.PlayByID(CS.GF2.Monopoly.MonopolyDefine.RandomPointAudioId)
    end
    self.stage1AudioPlay[index] = true
  end
  local listIndex2 = {}
  ...
  local setter2 = function(tempSelf, value)
    ...
    if self.stage2AudioPlay[index] == nil then
      AudioUtils.PlayByID(CS.GF2.Monopoly.MonopolyDefine.RandomPointAudioId)
    end
    self.stage2AudioPlay[index] = true
  end
  self.timer1 = TimerSys:DelayCall(0.5, function()
    self.tweenStep1 = LuaDOTweenUtils.ToOfInt(self, setter1, 1, #listIndex1, 0.5 * rate, function()
      self.tweenStep2 = LuaDOTweenUtils.ToOfInt(self, setter2, 1, #listIndex2, 1 * rate, function()
        for j = minPoint, maxPoint do
...
function ActivityTourPointRandomItem:RefreshResult()
  ...
  if self.bufPoint and self.bufPoint ~= 0 then
    self.timer3 = TimerSys:DelayCall(self.pointsAniLength, function()
      ...
      self.timer4 = TimerSys:DelayCall(0.5, function()
        self:PlayHideAni()
      end)
    end)
  else
