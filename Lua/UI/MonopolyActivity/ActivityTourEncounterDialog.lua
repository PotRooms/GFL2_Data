...
function ActivityTourEncounterDialog:AddBtnListener()
  ...
  UIUtils.GetButtonIntervalListener(self.ui.mBtn_Start.gameObject, ActivityTourGlobal.ButtonBlockTime).onClick = function()
...
function ActivityTourEncounterDialog:RefreshStageInfo()
  ...
  local monsterData = MonopolyWorld.MpData:GetMonsterData(self.monsterId)
...
function ActivityTourEncounterDialog:RefreshSpine()
  ...
  local gunData = NetCmdTeamData:GetGunByID(gunId)
  if not gunData.CharacterData then
    return
  end
  ResSys:GetSpineUIObjectAsync(gunData.CharacterData.Spine, function(path, go, data)
    if go ~= nil then
      self.spine = go
...
function ActivityTourEncounterDialog:RefreshActionOrder()
  ...
  local isShow = MonopolyWorld.MpData.configData.battle_order == 0
  if not isShow then
    return
  end
