...
function ActivityTourSettlementBase:OnBtnClose()
  ...
  self:DelayCall(length, function()
    self:CloseSelf()
  end)
end
...
function ActivityTourSettlementBase:RefreshTaskInfo()
  local config = MonopolyWorld.mapData
  if not config then
    return
  end
  if config.win_condition.Count <= 0 then
    setactive(self.ui.mTrans_Task.gameObject, false)
    return
  end
  setactive(self.ui.mTrans_Task.gameObject, true)
  local index = 1
  for i = 0, config.win_condition.Count - 1 do
    local taskId = config.win_condition[i]
