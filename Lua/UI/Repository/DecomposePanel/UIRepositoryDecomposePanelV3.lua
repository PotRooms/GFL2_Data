...
function UIRepositoryDecomposePanelV3:OnAwake(root, data)
  ...
  function self.virtualList.itemCreated(loopGridViewItem)
    local item = self:ItemProvider(loopGridViewItem)
    return item
  end
...
function UIRepositoryDecomposePanelV3:SetSelectNum(item, count)
  if count == 0 then
    item:SetSelectNum(0)
    ...
    if not self.contentItem[self.selectFrameIndex].mWeaponPartsData.IsLocked then
      for i = 1, #self.selectIndexTable do
        if self.selectIndexTable[i].Index == self.selectFrameIndex then
          ...
          self:SetSelectNum(self.contentItem[self.selectFrameIndex], 0)
          break
        end
      end
    end
...
function UIRepositoryDecomposePanelV3:OnClickTab(tabId, noInitSort)
  self.curTabId = tabId
  self.selectIndex = -1
  self.selectIndexTable = {}
  self.toRemovePartList = {}
  ...
  if tabId ~= UIRepositoryGlobal.PanelType.ItemPanel and not noInitSort then
    self:InitSortPart()
  end
  self.itemList = {}
...
function UIRepositoryDecomposePanelV3:ItemProvider(loopGridViewItem)
  local itemView = UICommonItem.New()
  itemView:InitCtrlWithNoInstantiate(loopGridViewItem, false)
  loopGridViewItem.data = itemView
end

function UIRepositoryDecomposePanelV3:ItemRenderer(index, renderData)
  local data = self.itemList[index + 1]
  local item = renderData.data
  local instanceId = renderData.gameObject:GetInstanceID()
  ...
  elseif curItemTabId == UIRepositoryGlobal.PanelType.WeaponPanel then
    ...
    item:SetLock(curItem.attr.IsLocked)
  ...
  if self:IsUnstackMode() then
    ...
    for _, v in pairs(self.selectIndexTable) do
      if v.Index == index + 1 then
        if curItemTabId == UIRepositoryGlobal.PanelType.WeaponParts then
          item:SetSelect(1 <= v.Count)
          break
        end
        self:SetSelectNum(item, v.Count)
        break
      end
    end
  end
...
function UIRepositoryDecomposePanelV3:OnClickStackItem(index, item)
  ...
  if isSelected then
    ...
    self:SetSelectNum(item, self.sliderMaxNum)
  end
  item:SetSelectShow(true)
end
...
function UIRepositoryDecomposePanelV3:OnClickWeaponPartItem(index, item, ignoreTable)
  ...
  self:RefreshSlider()
  local showCalibrationBtn = weaponPartsData ~= nil and self.curTabId == UIRepositoryGlobal.PanelType.WeaponParts and weaponPartsData:CanCalibration()
  setactive(self.ui.mBtn_Calibration.gameObject, showCalibrationBtn)
  if showCalibrationBtn then
    local openCalibrationDialog = function()
      local callback = function()
        self.itemList[index].curItem = NetCmdWeaponPartsData:GetWeaponModById(weaponPartsData.id)
        weaponPartsData = self.itemList[index].curItem
      ...
      local param = {gunWeaponModData = weaponPartsData, callback = callback}
  ...
  local modPowerData = weaponPartsData.ModPowerData
  local groupSkillData = weaponPartsData.GroupSkillData
  local PowerSkillCsData = weaponPartsData.PowerSkillCsData
  local ProficiencySkillData = weaponPartsData.ProficiencySkillData
  local flag = false
  if weaponPartsData.ModEffectTypeData.EffectId == UIWeaponGlobal.ModEffectType.Cover and weaponPartsData.ModPowerData then
    CS.GunWeaponModData.SetModPowerDataNameWithGroupNum(self.ui.mText_MakeUpName, self.ui.mText_MakeUpLv, weaponPartsData.ModPowerData, weaponPartsData)
  ...
  if weaponPartsData.ModEffectTypeData.EffectId == UIWeaponGlobal.ModEffectType.Ambush or weaponPartsData.ModEffectTypeData.EffectId == UIWeaponGlobal.ModEffectType.Armor then
    if weaponPartsData.ExtraCapacity ~= 0 or groupSkillData ~= nil or 0 < preficAttList then
  ...
  setactive(self.ui.mTrans_SpIcon, weaponPartsData.IsSp)
  setactive(self.ui.mTrans_Special, flag)
  setactive(self.ui.mTrans_MakeUp, groupSkillData ~= nil or 0 < preficAttList)
...
function UIRepositoryDecomposePanelV3:OnShowPublicSkillItem(index, item, ignoreTable)
  ...
  if itemTabData ~= nil then
    ...
    if skillDisplayData then
      local skillDes = CS.GF2.Battle.SkillUtils.GetSkillDes(skillDisplayData.id, false, 0, nil)
      self.ui.mTxt_TalentSkill.text = skillDes
...
function UIRepositoryDecomposePanelV3:GetWeaponPartList(list, partRankType)
  local weaponPartsList = NetCmdWeaponPartsData:GetAllMods()
  weaponPartsList = NetCmdWeaponPartsData:SortGunWeaponModDataDefault(weaponPartsList)
  for i = weaponPartsList.Count - 1, 0, -1 do
    local itemTabData = weaponPartsList[i].ItemData
...
function UIRepositoryDecomposePanelV3:QicklySelectWeaponParts()
  ...
  elseif preCount == #self.selectIndexTable and preCount ~= self:GetDisposalLimit() and not self:CheckMaxDecomposeProgress() then
    UIUtils.PopupHintMessage(1107)
    if #self.selectIndexTable > 0 and 0 < #self.contentItem then
      local index = 0
      for _, data in pairs(self.selectIndexTable) do
        if index == 0 or index > data.Index then
          index = data.Index
...
function UIRepositoryDecomposePanelV3:RefreshSlider()
  ...
  if self:IsUnstackMode() then
    ...
    if self.selectFrameIndex ~= -1 then
      self:SetSelectNum(self.contentItem[self.selectFrameIndex], self.curDecomposeNum)
    end
        ...
        if DarkNetCmdMakeTableData.FullDecompose then
          if ret == ErrorCodeSuc then
            UISystem:OpenCommonReceivePanel({
              function()
                self:OnClickTab(self.curTabId, true)
...
function UIRepositoryDecomposePanelV3:InitSortPart()
  ...
  self.ui.mTrans_TypeContent = self.uiBind.mScrollListChild_ScreenList.transform
  local tmpParent = self.uiBind.mScrollListChild_ScreenList
  ...
  for i = 1, #self.QuickSortType[self.curTabId] do
    ...
    if obj then
      ...
      sort.obj = obj
      sort.uiGrp = {}
      self:LuaUIBindTable(obj, sort.uiGrp)
      sort.grpset = sort.uiGrp.mTrans_GrpSel
      sort.btnSort = sort.uiGrp.mBtn_Select
      sort.txtName = sort.uiGrp.mText_SuitName
      sort.sortType = i
      ...
      self.textcolor = sort.uiGrp.textColor
...
function UIRepositoryDecomposePanelV3:ShowPropertyNow(propertyId)
  ...
  local usedIndex = 1
  for j = startPropertyId, endPropertyId - 1 do
...
function UIRepositoryDecomposePanelV3:OnTop()
  ...
  if self.curTabId == UIRepositoryGlobal.PanelType.WeaponParts then
    local weaponPartsData = self.itemList[self.selectFrameIndex].curItem
    local showCalibrationBtn = weaponPartsData ~= nil and weaponPartsData:CanCalibration()
    setactive(self.ui.mBtn_Calibration.gameObject, showCalibrationBtn)
  end
end
...
function UIRepositoryDecomposePanelV3:GetWeaponOfferExpDic()
  ...
  for _, item in pairs(self.itemList) do
    self.weaponPartExpList[item.curItem.id] = item.curItem:GetStcExp()
  end
end
