...
function UIRepositoryGunCorePanel:ItemRenderer(index, renderData)
  ...
  local count = NetCmdItemData:GetItemCountById(dataId)
  local customOnClick
  if self.currentIndex == 1 then
    item:SetItemData(dataId, count, nil, self.needGetList, nil, nil, nil, customOnClick)
  ...
  elseif self.currentIndex == 2 then
    ...
    local itemNum = NetCmdItemData:GetItemCount(dataId)
    local needCount = 0
...
function UIRepositoryGunCorePanel:RefreshItemList()
  ...
  if self.TopTabTable[2] ~= nil then
...
function UIRepositoryGunCorePanel:OnClickTypeDropdownItem(itemIndex, keep)
  self.pendingList = {}
