...
function UIRepositoryCommonPanel:Show()
  ...
  NetCmdItemData:ViewGiftPickRedPoint()
  self.currentIndex = 0
  ...
  self:OnClickTypeDropdownItem(1, true)
...
function UIRepositoryCommonPanel:RefreshItemList()
  ...
  self.parent.ui.mTrans_Empty.text = string_format(TableData.GetHintById(1109), topTagData.title.str)
...
function UIRepositoryCommonPanel:OnClickTypeDropdownItem(itemIndex, keep)
...
function UIRepositoryCommonPanel:ItemRenderer(index, renderDataItem)
  ...
  if data.type == GlobalConfig.ItemType.GiftPick then
    local custOnclick = function()
      UIManager.OpenUIByParam(UIDef.UIRepositoryBoxDialog, data)
    end
  ...
  elseif data.type == GlobalConfig.ItemType.Random then
    local custOnclick = function()
      UIManager.OpenUIByParam(UIDef.UIRepositoryUnSelectItemBoxDialog, data)
    end
    local t = TableData.GlobalSystemData.BackpackJumpSwitch == 1
    item:SetItemData(data.id, count, false, t, count, nil, nil, custOnclick, nil, true)
  elseif data.type == GlobalConfig.ItemType.WeaponCod or data.type == GlobalConfig.ItemType.WeaponCod1 then
    local custOnclick = function()
      UIManager.OpenUIByParam(UIDef.UIRepositoryUnitePartBoxDialog, data)
    end
    local t = TableData.GlobalSystemData.BackpackJumpSwitch == 1
    item:SetItemData(data.id, count, false, t, count, nil, nil, custOnclick, nil, true)
  elseif GlobalConfig.CanUseItemType[data.type] ~= nil then
