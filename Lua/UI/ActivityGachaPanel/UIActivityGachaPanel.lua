...
function UIActivityGachaPanel:OnInit(root, data)
  ...
  self.spritePath = data.spritePath
...
function UIActivityGachaPanel:SetTopResourceBar()
  local config = TableData.listActivityGachaConfigDatas:GetDataById(self.mData.id)
  if not config then
    return
  end
  local resources = tostring(config.gacha_item)
  self.csPanel.TopResourceBar.UITopResourceBar:Init(true)
  self.csPanel.TopResourceBar.UITopResourceBar:UpdateCurrencyContent(resources)
end
...
function UIActivityGachaPanel:OnClose()
  ...
  ResourceManager:UnloadAssetFromLua(self.mDecoAsset)
  self.mDecoAsset = nil
  if self.mDecoItem ~= nil then
    gfdestroy(self.mDecoItem)
    self.mDecoItem = nil
  end
end
...
function UIActivityGachaPanel:RefreshSprite()
  ...
  self.ui.mImg_Bg.sprite = IconUtils.GetAtlasV2(bgPath, ActivityGachaGlobal.SpriteName.MainBg)
  self.ui.mImg_TopBg.sprite = IconUtils.GetActivityThemeSprite(spritePath .. ActivityGachaGlobal.SpriteName.TopBg)
  self.ui.mImg_TopFg.sprite = IconUtils.GetActivityThemeSprite(spritePath .. ActivityGachaGlobal.SpriteName.TopFg)
  self.ui.mImg_GachaOneBg.sprite = IconUtils.GetActivityThemeSprite(spritePath .. ActivityGachaGlobal.SpriteName.GachaOneBg)
  self.ui.mImg_GachaOneHl.sprite = IconUtils.GetActivityThemeSprite(spritePath .. ActivityGachaGlobal.SpriteName.GachaHl)
  self.ui.mImg_GachaManyBg.sprite = IconUtils.GetActivityThemeSprite(spritePath .. ActivityGachaGlobal.SpriteName.GachaManyBg)
  self.ui.mImg_GachaManyHl.sprite = IconUtils.GetActivityThemeSprite(spritePath .. ActivityGachaGlobal.SpriteName.GachaHl)
  self.ui.mImg_TurnBg.sprite = IconUtils.GetActivityThemeSprite(spritePath .. ActivityGachaGlobal.SpriteName.TurnBg)
  self.ui.mImg_TextDeco.sprite = IconUtils.GetActivityThemeSprite(spritePath .. ActivityGachaGlobal.SpriteName.TextDeco)
  self.ui.mText_List.color = ActivityGachaGlobal.TextListColor[self.spritePath]
end

function UIActivityGachaPanel:LoadDeco()
  if self.spritePath == ActivityGachaGlobal.DaiyanPath then
    return
  end
  if self.mDecoAsset == nil then
    ResSys:LoadUIAssetAsync("ActivityTheme/" .. self.spritePath .. "/" .. self.spritePath .. "GachaDecoItem.prefab", function(s, o, arg)
      if o then
        self:AddAsset(o)
        self.mDecoAsset = o
        self.mDecoItem = instantiate(self.mDecoAsset, self.ui.mTrans_GrpDeco)
      end
    end)
  end
end
...
function UIActivityGachaPanel:RefreshActivityInfo()
  ...
  local activityEntranceData = TableData.listActivityEntranceDatas:GetDataById(planData.args[0])
  local activityModuleData = TableData.listActivityModuleDatas:GetDataById(activityEntranceData.module_id)
  self.moduleKey = 4001
  for k, v in pairs(activityModuleData.activity_submodule) do
    if k == 4001 then
      self.moduleValue = v
      break
    end
  end
  CSUIUtils.GetAndSetActivityHintText(self.mUIRoot, self.actId, 2, self.moduleKey, self.moduleValue)
  if planData then
    ...
    self.ui.mText_Time.text = string_format(TableData.GetActivityHint(270132, self.actId, 2, self.moduleKey, self.moduleValue), openTime, closeTime)
  else
...
function UIActivityGachaPanel:RefreshTop()
  ...
  self.ui.mText_TopNum.text = TableData.GetActivityHint(270117 + self.curSelGroup - 1, self.actId, 2, self.moduleKey, self.moduleValue)
  self.ui.mImg_TopNum.sprite = IconUtils.GetAtlasV2(ActivityGachaGlobal.IconRootPath .. self.spritePath, ActivityGachaGlobal.SpriteName.GachaGroupIcon .. self.curSelGroup)
end
...
function UIActivityGachaPanel:RefreshReward()
  ...
  local data = self.mData.data[self.curSelGroup]
  self.targetRewardDetail:SetData(ActivityGachaGlobal.TargteType, data and data[ActivityGachaGlobal.TargteType] or nil, self.actId, self.moduleKey, self.moduleValue)
  self.normalRewardDetail:SetData(ActivityGachaGlobal.NormalType, data and data[ActivityGachaGlobal.NormalType] or nil, self.actId, self.moduleKey, self.moduleValue)
end
...
function UIActivityGachaPanel:RefreshRound()
  ...
  for i = 1, self.maxRound do
    ...
    item:SetData(self.mData.id, i, data, self.spritePath, self.actId, self.moduleKey, self.moduleValue, function(group)
...
function UIActivityGachaPanel:RefreshDropDownMenu()
  ...
  for i = 0, data.continuity.Count - 1 do
    ...
    itemBinding.mText_SuitName.text = string_format(TableData.GetActivityHint(270122, self.actId, 2, self.moduleKey, self.moduleValue), data.continuity[i])
...
function UIActivityGachaPanel:RefreshStateDoingButton()
  ...
  self.ui.mText_LeftConsume.text = string_format(TableData.GetActivityHint(270176, self.actId, 2, self.moduleKey, self.moduleValue), config.gacha_cost)
  ...
  self.ui.mText_RightConsume.text = string_format(TableData.GetActivityHint(270176, self.actId, 2, self.moduleKey, self.moduleValue), costNum)
  if LuaUtils.IsNullOrDestroyed(self.btnOne) then
    self.btnOne = UIActivitieGachaRaffleItem.New()
    self.btnOne:InitCtrl(self.ui.mTrans_BtnOne.gameObject)
  end
  self.btnOne:SetData(1, function()
    if curNum < config.gacha_cost then
      CS.PopupMessageManager.PopupString(string_format(TableData.GetActivityHint(270131, self.actId, 2, self.moduleKey, self.moduleValue), UIUtils.GetItemName(config.gacha_item)))
      return
    end
    self:DoRaffle(1)
  end)
  ...
  if self:IsAllTargetRewardGet(self.curSelGroup, ActivityGachaGlobal.TargteType) and self:IsAllTargetRewardGet(self.curSelGroup, ActivityGachaGlobal.NormalType) then
    ...
    self.ui.mText_Tip.text = TableData.GetActivityHint(270114, self.actId, 2, self.moduleKey, self.moduleValue)
  end
end
...
function UIActivityGachaPanel:RefreshStateCloseButton()
  ...
  setactive(self.ui.mTrans_GrpConsume.gameObject, false)
  setactive(self.ui.mTrans_GrpBtn.gameObject, false)
  self.ui.mText_Tip.text = TableData.GetActivityHint(270115, self.actId, 2, self.moduleKey, self.moduleValue)
end
...
function UIActivityGachaPanel:RefreshStateNotOpenButton()
  ...
  setactive(self.ui.mTrans_GrpConsume.gameObject, false)
  setactive(self.ui.mTrans_GrpBtn.gameObject, false)
  self.ui.mText_Tip.text = TableData.GetActivityHint(270116, self.actId, 2, self.moduleKey, self.moduleValue)
end
...
function UIActivityGachaPanel:CreateBtnMany()
  if LuaUtils.IsNullOrDestroyed(self.btnMany) then
    ...
    self.btnMany:InitCtrl(self.ui.mTrans_BtnMany.gameObject)
  end
end
...
function UIActivityGachaPanel:DoRaffle(count)
  ...
  if self:IsAllTargetRewardGet(self.curSelGroup, ActivityGachaGlobal.TargteType) and self.curSelGroup < self.maxRound then
    if NetCmdActivityGachaData.ShowRaffleTip then
      ...
      todayTipsParam[1] = string_format(TableData.GetActivityHint(270124, self.actId, 2, self.moduleKey, self.moduleValue), TableData.GetActivityHint(270014, self.actId, 2, self.moduleKey, self.moduleValue))
    ...
    if config then
      ...
      if unused_item_id.Count > 0 then
        for k, v in pairs(unused_item_id) do
          if 0 < k then
            MessageBox.ShowMidBtn(TableData.GetActivityHint(64, self.actId, 2, self.moduleKey, self.moduleValue), TableData.GetActivityHint(270123, self.actId, 2, self.moduleKey, self.moduleValue), nil, nil, function()
              UIManager.CloseUI(UIDef.UIActivityGachaPanel)
...
function UIActivityGachaPanel:DealAfterCloseRewardDialog()
  ...
  if self.isOpenNewGroup then
    if self:IsAllTargetRewardGet(self.curSelGroup, ActivityGachaGlobal.NormalType) and self:IsAllTargetRewardGet(self.curSelGroup, ActivityGachaGlobal.TargteType) then
      ...
      CS.PopupMessageManager.PopupStateChangeString(TableData.GetActivityHint(270114, self.actId, 2, self.moduleKey, self.moduleValue))
    else
      if self.curSelGroup < self.maxRound then
        self.curSelGroup = self.curSelGroup + 1
      end
      local tip = string_format(TableData.GetActivityHint(270125, self.actId, 2, self.moduleKey, self.moduleValue), TableData.GetActivityHint(270014, self.actId, 2, self.moduleKey, self.moduleValue))
