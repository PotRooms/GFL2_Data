...
function UIActivityGachaReceiveDialog:OnInit(root, data)
  ...
  self.randItems = data[7]
  CSUIUtils.GetAndSetActivityHintText(self.mUIRoot, data[8], 2, data[9], data[10])
end
        ...
        for _, data in pairs(showData) do
          ...
          if itemData then
            if rewardData.type == ActivityGachaGlobal.TargteType then
              if self.randItems ~= nil and self.randItems:ContainsKey(data.id) then
                for _, key in pairs(self.randItems[data.id].Items_.Keys) do
                  UIUtils.AddDrop(targetData, {
                    itemId = key,
                    itemNum = self.randItems[data.id].Items_[key]
                  })
                end
                break
              end
