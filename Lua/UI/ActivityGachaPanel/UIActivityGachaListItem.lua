...
function UIActivityGachaListItem:SetData(type, rewardData, actId, moduleKey, moduleValue)
  self.actId = actId
  self.moduleKey = moduleKey
  self.moduleValue = moduleValue
  CSUIUtils.GetAndSetActivityHintText(self.mUIRoot, actId, 2, moduleKey, moduleKey)
  self.ui.mText_Name.text = type == ActivityGachaGlobal.TargteType and TableData.GetActivityHint(270014, self.actId, 2, self.moduleKey, self.moduleValue) or TableData.GetActivityHint(270015, self.actId, 2, self.moduleKey, self.moduleValue)
  ...
  if isInfinite then
    self.ui.mText_Residue.text = TableData.GetActivityHint(270316, self.actId, 2, self.moduleKey, self.moduleValue)
  else
    self.ui.mText_Residue.text = string_format(TableData.GetActivityHint(270113, self.actId, 2, self.moduleKey, self.moduleValue), curNum .. "/" .. totalNum)
  end
  self.ui.mText_Num.text = string_format(TableData.GetActivityHint(270113, self.actId, 2, self.moduleKey, self.moduleValue), curNum)
  for i = 1, #rewardData do
    ...
    if rewardData[i].totalNum >= 999999 then
      binding.mText_Num.text = TableData.GetActivityHint(270316, self.actId, 2, self.moduleKey, self.moduleValue)
    else
      binding.mText_Num.text = string_format(TableData.GetActivityHint(270113, self.actId, 2, self.moduleKey, self.moduleValue), rewardData[i].curNum)
    end
    binding = {}
