...
function AmoWishListItem:ctor(prefab)
  self.obj = prefab
  self:InitCtrl(prefab)
end

function AmoWishListItem:__InitCtrl()
end

function AmoWishListItem:InitCtrl(prefab)
  self.ui = {}
...
function AmoWishListItem:SetData(id, isLast, planActivityData)
  ...
  local isMainQuestRewardGet = NetCmdActivityAmoData:HasMainQuestRewardGet(self.mData.id)
  local isUnlock = NetCmdActivityAmoData:GetMainQuestUnlock(self.mData.id)
