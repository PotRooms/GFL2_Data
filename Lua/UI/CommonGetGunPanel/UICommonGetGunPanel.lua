...
function UICommonGetGunPanel:OnCameraStart()
  return 0.01
end

function UICommonGetGunPanel:OnClose()
  if not self.isGacha then
    self.isOverFlow = false
    local notShowItemList = TableData.GlobalSystemData.ItemGetCommonWindowNoshow
    if NetCmdItemData:GetUserDropCache().Count > 0 then
      local itemlist = {}
      for i = 0, NetCmdItemData:GetUserDropCache().Count - 1 do
        ...
        if notShowItemList:Contains(typeID) == false then
          if 0 < item.TranItems.Count then
            for id, num in pairs(item.TranItems) do
              table.insert(itemlist, {ItemId = id, ItemNum = num})
            end
          else
...
function UICommonGetGunPanel:OnInit(root, data)
  ...
  SceneSys:OpenGunGetScene(function(obj)
    ...
    local type = SceneSys.CurrentSingleScene:GetSceneType()
    self.mGunGetShow = obj
    if type == CS.EnumSceneType.Gacha or type == CS.EnumSceneType.HallScene then
...
function UICommonGetGunPanel:PlayFade2()
  ...
  if self.skipPanel ~= nil then
    ...
    setactive(self.skipPanel.ui.mBtn_IconSkip.gameObject, self.isGacha)
  end
...
function UICommonGetGunPanel:ShowSSRTimeline()
  if self.mGunGetShow ~= nil then
    ...
    if SceneSys.CurrentSingleScene:GetSceneType() == CS.EnumSceneType.Battle then
      SceneSys.CurrentSingleScene:EnableSceneObjs(false)
    end
    if SceneSys.CurrentSingleScene:GetSceneType() == CS.EnumSceneType.HallScene then
      SceneSys.CurrentSingleScene:EnableGunGetSceneObjs(true)
    end
    if SceneSys.CurrentSingleScene:GetSceneType() == CS.EnumSceneType.Gacha then
      SceneSys.CurrentSingleScene:EnableVirtualCamera(true)
    end
  end
...
function UICommonGetGunPanel:OnShowStart()
  ...
  if self.needSkip then
    ...
    if self.skipPanel == nil and not self.isGacha then
...
function UICommonGetGunPanel:OnEnd(skipPanel)
  ...
  if SceneSys.CurrentSingleScene:GetSceneType() == CS.EnumSceneType.Gacha then
    SceneSys.CurrentSingleScene:SetActiveScene()
  end
...
function UICommonGetGunPanel:UpdateItemPanel(data)
  ...
  if self.newGunTimer ~= nil then
    self.newGunTimer:Stop()
    self.newGunTimer = nil
  end
  ...
  if self.itemData.type == GlobalConfig.ItemType.Weapon then
    ...
    if self.skipPanel ~= nil then
      ...
      self.newGunTimer = TimerSys:DelayCall(1, function()
        if self.skipPanel ~= nil then
    ...
    if not self.isGacha then
      local list = NetCmdItemData:GetUserDropWeaponCache()
      if 0 < list.Count then
        local weapon = list[0]
        if 0 < weapon.OverflowNum and (self.isOverFlow == nil or self.isOverFlow == false) then
          CS.PopupMessageManager.PopupDownLeftTips(self.item.ItemId, 1)
...
function UICommonGetGunPanel:UpdateGunContent(gunData)
  if gunData then
    ...
    self.ui.mText_ChrType.text = "/" .. CS.GunCmdData.GetGunSecondDutyStrByGunId(gunData.id)
    ...
    function self.loadTimeLine()
      if gunData.gacha_get_timeline ~= "" then
        if SceneSys.CurrentSingleScene:GetSceneType() == CS.EnumSceneType.Gacha then
          SceneSys.CurrentSingleScene:HideDefaultObjs()
        end
        if not CS.LuaUtils.IsNullOrDestroyed(self.mGunGetShow) then
          ...
          if SceneSys.CurrentAdditiveSceneType == CS.EnumSceneType.GunGetShowSSR then
          ...
          setactive(self.ui.mTrans_BlackMask, true)
          UIManager.OpenUIByParam(UIDef.UIGachaDialogPanel, {
            gunData,
            function()
              setactive(self.ui.mTrans_BlackMask, false)
              if self.skipPanel ~= nil then
                setactive(self.skipPanel.ui.mBtn_BgSkip.gameObject, true)
              end
              if SceneSys.CurrentAdditiveSceneType == CS.EnumSceneType.GunGetShowSSR then
          ...
          gfwarning("GunGetShow\228\184\186\231\169\186\230\136\150\229\183\178\232\162\171\233\148\128\230\175\129\239\188\129")
        end
      end
    end
...
function UICommonGetGunPanel:UpdateWeaponContent(weaponData)
  ...
  setactive(self.ui.mImg_WeaponIcon, false)
  IconUtils.GetAtlasSpriteAsyc("Icon/Weapon/" .. weaponData.res_code .. "_1024", function(s, o, arg)
    if o then
      self.ui.mImg_WeaponIcon.sprite = o
    else
      self.ui.mImg_WeaponIcon.sprite = IconUtils.GetWeaponSpriteL(weaponData.res_code)
    end
    setactive(self.ui.mImg_WeaponIcon, true)
  end)
  setactive(self.ui.mImg_WeaponType, false)
  IconUtils.GetAtlasSpriteAsyc("Icon/GunTypeIcon/" .. weaponTypeData.icon, function(s, o, arg)
    if o then
      self.ui.mImg_WeaponType.sprite = o
    else
      self.ui.mImg_WeaponType.sprite = IconUtils.GetGunGashaponPic(weaponTypeData.icon)
    end
    setactive(self.ui.mImg_WeaponType, true)
  end)
