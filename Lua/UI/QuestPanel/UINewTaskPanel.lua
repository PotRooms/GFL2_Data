...
function UINewTaskPanel:AddBtnListener()
  ...
  UIUtils.GetButtonListener(self.ui.mTrans_TopSee.gameObject).onClick = function()
    UIManager.OpenUI(UIDef.UINewTaskRewardDialog)
  end
end
...
function UINewTaskPanel:UpdateShow()
  ...
  if receivedCount == totalCount then
    if NetCmdQuestData:CheckNewbiePhaseIsReceived(curPhase) then
      ...
      if curPhase == self:GetMaxPhaseId() then
  ...
  self.ui.mText_PhaseProgress.text = string_format(TableData.GetHintById(112047), string.format("%02d", curPhase))
...
function UINewTaskPanel:UpdateMileQuest()
  ...
  if tmp then
    ...
    if itemData.type == GlobalConfig.ItemType.Weapon then
      ...
      self.ui.mText_MileageReward.text = itemData.name.str .. "\195\151" .. itemNum
    end
  end
...
function UINewTaskPanel:UpdatePhaseRewardList()
  ...
  for index, pair in pairs(rewards) do
    ...
    itemView:SetRewardEffect(id == 1015)
