...
function UIQuestDailyRewardPoint:Refresh()
  ...
  self.dailyRewardDataList = NetCmdQuestData:GetDailyRewardDataList()
  self.isActive = self.havePoint >= self.curFlagPoint
  if self.havePoint >= self.curFlagPoint then
    if NetCmdQuestData:IsDailyRewardReceive(self.dailyRewardDataList[self.index - 1].Id) then
