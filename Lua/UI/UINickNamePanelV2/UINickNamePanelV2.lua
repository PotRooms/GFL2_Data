...
function UINickNamePanelV2:OnConfirmName()
  ...
  local isOverLen = LuaUtils.IsOverStringLen(strName, 7)
  if isOverLen then
    self.ui.mInputField.text = LuaUtils.LimitString(strName, 7)
    UIUtils.PopupHintMessage(60078)
    return
  end
...
function UINickNamePanelV2:OnClickSex(type)
  ...
  if nowTime - self.lastClickSex < 0.5 then
    return
  end
  self.lastClickSex = nowTime
...
function UINickNamePanelV2:UpdateBirthDay(birth)
  ...
  if self.curDate < 0 then
    ...
    self.ui.mText_NumMonth.text = string.format("%02d", tostring(month))
    self.ui.mText_NumDay.text = string.format("%02d", tostring(day))
  end
end
