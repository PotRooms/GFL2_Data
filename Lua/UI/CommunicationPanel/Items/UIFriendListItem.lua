...
function UIFriendListItem:SetData(data, type)
  if data then
    ...
    self.ui.mImg_AvatarFrame.sprite = IconUtils.GetPlayerAvatarFrame(self.playerInfo.IconFrame)
    ...
    if self.playerInfo.ReputationTitle ~= nil and self.playerInfo.ReputationTitle ~= 0 and self.playerInfo.ReputationTitle ~= TableData.GlobalSystemData.PlayerTitleDefault then
      ...
      self.titleObj.transform:GetChild(0):GetComponent(typeof(CS.UnityEngine.UI.Image)).sprite = IconUtils.GetPlayerTitlePic(TableData.listIdcardTitleDatas:GetDataById(self.playerInfo.ReputationTitle).icon)
      ...
      self.titleObj.transform:GetChild(1):GetComponent(typeof(CS.UnityEngine.UI.Text)).text = "\230\178\161\230\156\137\231\167\176\229\143\183"
    end
...
function UIFriendListItem:OnChangeNoteCallback(ret)
  if ret == ErrorCodeSuc then
    gfdebug("\228\191\174\230\148\185\230\136\144\229\138\159")
  else
    gfdebug("\228\191\174\230\148\185\229\164\177\232\180\165")
  end
  ...
  if self.playerInfo then
    if self.playerInfo.IsFriend then
      NetCmdFriendData:SendSocialFriendSearch(tostring(self.playerInfo.UID), function()
        ...
        LuaUtils.OpenUIPlayerInfoDialog(self.playerInfo)
      end)
    else
      LuaUtils.OpenUIPlayerInfoDialog(self.playerInfo)
    end
