...
function UIBattlePassBoughtDialog:OnShowFinish()
  if self.mStoreGoodData ~= nil and self.mStoreGoodData.price_type > 0 then
    ...
    self.ui.mText_Num.color = costItemNum < self.mStoreGoodData.price * self.mDetalNum and ColorUtils.RedColor or CS.GF2.UI.UITool.StringToColor("325563")
    if self.mIsBuyLevel == false then
      local costItemNum = NetCmdItemData:GetItemCountById(self.mStoreGoodData.price_type)
...
function UIBattlePassBoughtDialog:SetExpReward(buyNum)
  ...
  if self.mStoreGoodData ~= nil and 0 < self.mStoreGoodData.price_type then
    local costItemNum = NetCmdItemData:GetItemCountById(self.mStoreGoodData.price_type)
    self.ui.mText_Num.color = costItemNum < self.mStoreGoodData.price * buyNum and ColorUtils.RedColor or CS.GF2.UI.UITool.StringToColor("325563")
  end
end

function UIBattlePassBoughtDialog:SetLevelReward(deltaLevel, isBase)
  self.mDetalNum = deltaLevel
  ...
  if self.mStoreGoodData ~= nil and self.mStoreGoodData.price_type > 0 then
    ...
    self.ui.mImg_Icon.sprite = IconUtils.GetItemIcon(stcData.icon)
    local costItemNum = NetCmdItemData:GetItemCountById(self.mStoreGoodData.price_type)
    self.ui.mText_Num.color = costItemNum < self.mStoreGoodData.price * deltaLevel and ColorUtils.RedColor or CS.GF2.UI.UITool.StringToColor("325563")
  end
  ...
  local OnBuyClick = function()
    ...
    if self.mStoreGoodData ~= nil and 0 < self.mStoreGoodData.price_type then
      ...
      if costItemNum < self.mStoreGoodData.price * buyNum then
        if self.mStoreGoodData.price_type == 1 then
          UISystem:JumpByID(3)
          return
        end
...
function UIBattlePassBoughtDialog:ItemBaseRenderer(index, renderData)
  ...
  local itemTabData = TableData.GetItemData(itemData.itemId)
  local isShowEffect = itemTabData.type == GlobalConfig.ItemType.GiftPick
  item:SetRewardEffect(isShowEffect)
end
