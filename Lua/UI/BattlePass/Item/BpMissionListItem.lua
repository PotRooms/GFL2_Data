...
function BpMissionListItem:ctor(go, parentCls)
...
function BpMissionListItem:Release()
  ...
  self.onReceiveCallback = nil
  self.ui = nil
  if self.timedelay1 ~= nil then
    self.timedelay1:Stop()
  end
end
...
function BpMissionListItem:Refresh()
  local showName = ""
  local bpTaskPackDataType = CS.LuaUtils.EnumToInt(self.bpTaskPackData.type)
  if bpTaskPackDataType == UIBattlePassGlobal.BpTaskTypeShow.Daily or bpTaskPackDataType == UIBattlePassGlobal.BpTaskTypeShow.TaskNew then
    showName = TableData.GetHintById(192073)
  elseif bpTaskPackDataType == UIBattlePassGlobal.BpTaskTypeShow.Weekly then
    showName = TableData.GetHintById(192074)
  elseif bpTaskPackDataType == UIBattlePassGlobal.BpTaskTypeShow.TaskCooperation then
  ...
  if self.bpTaskPackData.conditionNum ~= 0 then
    self.ui.mText_Progress.text = self.bpTaskPackData:GetRatioStr(self.ui.mText_Progress)
  ...
  if self.bpTaskPackData.isReceived then
    ...
    elseif self.bpTaskPackData.isCooperationTask then
      ...
      setactive(self.ui.mBtn_LookOver, true)
      setactive(self.ui.mBtn_LookOver.transform.parent, true)
    elseif self.bpTaskPackData.link == "" and self.bpTaskPackData.type ~= BpTaskType.TaskWeekly then
      setactive(self.ui.mTrans_Refresh, false)
    else
      if self.bpTaskPackData.type ~= BpTaskType.TaskWeekly then
        setactive(self.ui.mTrans_Refresh, false)
      end
...
function BpMissionListItem:onClickGoto()
  if self.parentCls ~= nil then
    self.parentCls:ShowFadeOut()
  end
  self.timedelay1 = TimerSys:DelayFrameCall(5, function()
    UISystem:JumpByID(tonumber(self.bpTaskPackData.link))
  end)
end
...
function BpMissionListItem:onClickRefresh()
  ...
  if 0 < nowRefreshCnt then
    MessageBox.ShowConfirmCancle(TableData.GetHintById(192025), textRefresh, nil, function()
