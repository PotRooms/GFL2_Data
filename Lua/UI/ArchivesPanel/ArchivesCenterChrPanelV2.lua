...
function ArchivesCenterChrPanelV2:OnAwake(root, data)
  ...
  UIUtils.GetButtonListener(self.ui.mBtn_BtnDorm.gameObject).onClick = function()
    ...
    if gun.isDormLockGun then
      ...
      NetCmdLoungeData:SetEnterSceneType(EnumSceneType.HallScene)
...
function ArchivesCenterChrPanelV2:OnInit(root, data)
  ...
  local gunDormData = NetCmdTeamData:GetGunByID(self.data.unit_id[0])
  if CS.AuditUtils:IsAudit() then
    setactivewithcheck(self.ui.mBtn_BtnDorm, gunDormData ~= nil and not gunDormData.isDormLockGun)
  else
    setactivewithcheck(self.ui.mBtn_BtnDorm, gunDormData ~= nil)
  end
end
...
function ArchivesCenterChrPanelV2:UpdateInfo()
  ...
  setactive(self.ui.mBtn_Record, isMeiLing == true)
  ...
  if self.data.unit_id.Count > 0 then
    setactive(self.ui.mTrans_PlotRedPoint.gameObject, self.data.story_open and 0 < NetCmdLoungeData:DormChrStoryRedPointByGunID(self.data.unit_id[0]))
    setactive(self.ui.mTrans_RedPoint.gameObject, 0 < NetCmdLoungeData:DormChrDailyRedPointByGunID(self.data.unit_id[0]))
  ...
  setactive(self.ui.mTrans_DormRedPoint.gameObject, isMeiLing == false and NetCmdLoungeData:GetDormSetRed(self.data.unit_id[0]))
