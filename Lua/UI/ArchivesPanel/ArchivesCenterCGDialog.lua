...
function ArchivesCenterCGDialog:InitInfoData()
  local imgPath = self.mData.img
  if self.mData.gender then
    if AccountNetCmdHandler.Gender == 1 then
      imgPath = self.mData.img .. "_Female"
    else
      imgPath = self.mData.img .. "_Male"
    end
  end
  self.ui.mImg_CG.sprite = ResSys:GetCGSprte(imgPath)
...
function ArchivesCenterCGDialog:LastPic()
  ...
  local imgPath = self.CGList[self.Index].img
  if self.CGList[self.Index].gender then
    if AccountNetCmdHandler.Gender == 1 then
      imgPath = self.CGList[self.Index].img .. "_Female"
    else
      imgPath = self.CGList[self.Index].img .. "_Male"
    end
  end
  self.ui.mImg_CG.sprite = ResSys:GetCGSprte(imgPath)
