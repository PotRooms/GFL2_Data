...
function ArchivesCenterCGPicturesItemV2:InitCtrlWithoutInstance(obj)
  self.ui = {}
...
function ArchivesCenterCGPicturesItemV2:SetData(data)
  ...
  local imgPath = data.img
  if data.gender then
    if AccountNetCmdHandler.Gender == 1 then
      imgPath = data.img .. "_Female"
    else
      imgPath = data.img .. "_Male"
    end
  end
  self.ui.mImg_CGPicture.sprite = ResSys:GetCGSprte(imgPath)
  ...
  if avgData then
    if NetCmdArchivesData:MainStoryIsUnLock(avgData.condition) then
      ...
      if stageData then
        self.ui.mText_Detail.text = string_format(avgData.text.str, stageData.code)
      end
