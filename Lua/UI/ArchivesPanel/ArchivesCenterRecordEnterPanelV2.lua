...
function ArchivesCenterRecordEnterPanelV2:UpdateBtnState()
  for k, v in ipairs(self.tabUIList) do
    local data = TableDataBase.listStoryRoomDatas:GetDataById(k)
    setactive(v.mTrans_Locked.gameObject, not AccountNetCmdHandler:CheckSystemIsUnLock(data.unlock))
  end
end
...
function ArchivesCenterRecordEnterPanelV2:OnClickTab(index)
  ...
  if self.currSelectIndex == 1 then
    ...
    if not self.btnStateList[self.currSelectIndex] then
      ...
      function self.ui.mVirtualListEx_Story.itemCreated(renderData)
        self:ItemStoryProvider(renderData)
      end
  ...
  elseif self.currSelectIndex == 2 then
    ...
    if not self.btnStateList[self.currSelectIndex] then
      ...
      function self.ui.mVirtualListEx_Hard.itemCreated(renderData)
        self:ItemHardProvider(renderData)
      end
...
function ArchivesCenterRecordEnterPanelV2:ItemStoryProvider(renderData)
...
function ArchivesCenterRecordEnterPanelV2:ItemHardProvider(renderData)
